...
public class MdlGlossaryService implements ToolSessionManager, ToolAdapterContentManager, IMdlGlossaryService,
	...
	params.put(CUSTOM_CSV_MAP_PARAM_EXT_LMS_ID, mdlglossary.getExtLmsId());

	String timestamp = Long.toString(new Date().getTime());
	params.put(EXT_SERVER_PARAM_TIMESTAMP, timestamp);

	ExtServerOrgMap serverMap = getExtServerOrgMap(mdlglossary.getExtLmsId());
	...
	ExtServerOrgMap extServerMap = getExtServerOrgMap(mdlGlossary.getExtLmsId());

	String extUserName = user.getLoginName().substring(extServerMap.getPrefix().length() + 1);

	try {
	    String outputServletUrl = getExtToolAdapterServletUrl(mdlGlossary.getExtLmsId());
	...
	return mdlGlossaryOutputFactory.getToolOutput(name, this, toolSessionId, learnerId, session.getMdlGlossary(),
		session.getExtSessionId());
    }
	...
	if (ret != null) {
	    ret += MdlGlossaryConstants.RELATIVE_SERVLET_URL;
	}
	return ret;
    }
	...
	if (mdlGlossary.getExtToolContentId() == null) {
	    ...
	    try {
		exportContentService.exportToolContent(toolContentId, mdlGlossary, mdlGlossaryToolContentHandler,
			rootPath);
		...
		String exportServletUrl = getExtToolAdapterServletUrl(mdlGlossary.getExtLmsId());
	...
	} else // use the normal LAMS method of importing activities from an xml file
	{
		...
		Object toolPOJO = exportContentService.importToolContent(toolContentPath,
			mdlGlossaryToolContentHandler, fromVersion, toVersion);
		if (!(toolPOJO instanceof MdlGlossary))
		    throw new ImportToolContentException(
			    "Import MdlGlossary tool content failed. Deserialized object is " + toolPOJO);
    ...
    public MdlGlossary getDefaultContent(String extLmsId) {
	MdlGlossary defaultContent = new MdlGlossary();
	...
	return integrationService.getMappedServers(MdlGlossaryConstants.TOOL_SIGNATURE);
    }
	...
	if (tool != null) {
		...
		for (int i = 0; i < mappedServers.length; i++) {
			...
			ExtServerToolAdapterMap serverToolMap = integrationService.getMappedServer(serverMap
				.getServerid(), MdlGlossaryConstants.TOOL_SIGNATURE);
	...
	try {
	    ITicket ticket = repositoryService
		    .login(credentials, MdlGlossaryToolContentHandler.repositoryWorkspaceName);
	    return ticket;

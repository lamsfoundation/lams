...
drop table if exists tl_mdscrm10_configuration;
drop table if exists tl_mdscrm10_mdlscorm;
drop table if exists tl_mdscrm10_session;
drop table if exists tl_mdscrm10_user;
create table tl_mdscrm10_configuration (uid bigint not null auto_increment, config_key varchar(30) unique, config_value varchar(255), primary key (uid))TYPE=InnoDB;
create table tl_mdscrm10_mdlscorm (uid bigint not null auto_increment, create_date datetime, update_date datetime, define_later bit, content_in_use bit, run_offline bit, tool_content_id bigint, ext_tool_content_id bigint, ext_user_name varchar(255), ext_course_id varchar(255), ext_section varchar(255), primary key (uid))TYPE=InnoDB;
...
INSERT INTO tl_mdscrm10_configuration (
	config_key, 
	config_value
)
VALUES(
	"toolAdapterServlet",
	"http://localhost:8000/lams2conf/tooladapter_mdscrm10"
);

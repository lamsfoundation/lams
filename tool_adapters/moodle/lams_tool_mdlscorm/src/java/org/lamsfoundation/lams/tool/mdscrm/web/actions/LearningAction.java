 ...
 *                name="learningForm"
 * @struts.action-forward name="mdlScorm" path="tiles:/learning/main"
...
public class LearningAction extends LamsDispatchAction {
    ...
    public static final String RELATIVE_TEACHER_URL = "mod/scorm/view.php?"; 
	...
	MdlScormUser mdlScormUser = getCurrentUser(toolSessionID);

	// check defineLater
	if (mdlScorm.isDefineLater()) {
	    return mapping.findForward("defineLater");
	}

	MdlScormDTO mdlScormDTO = new MdlScormDTO();
	request.setAttribute("mdlScormDTO", mdlScormDTO);

	// Set the content in use flag.
	if (!mdlScorm.isContentInUse()) {
	    mdlScorm.setContentInUse(new Boolean(true));
	    mdlScormService.saveOrUpdateMdlScorm(mdlScorm);
	}

	// check runOffline
	if (mdlScorm.isRunOffline()) {
	...
	if (mdlScorm.getExtToolContentId() != null) {
	    ...
	    throw new MdlScormException("External content id null for learner");
	}
	return null;
    }

    private MdlScormUser getCurrentUser(Long toolSessionId) {
	UserDTO user = (UserDTO) SessionManager.getSession().getAttribute(AttributeNames.USER);

	// attempt to retrieve user using userId and toolSessionId
	MdlScormUser mdlScormUser = mdlScormService.getUserByUserIdAndSessionId(new Long(user.getUserID().intValue()),
		toolSessionId);

	if (mdlScormUser == null) {
	    MdlScormSession mdlScormSession = mdlScormService.getSessionBySessionId(toolSessionId);
	    mdlScormUser = mdlScormService.createMdlScormUser(user, mdlScormSession);
	}
	...
	if (mdlScormUser != null) {
	    mdlScormUser.setFinishedActivity(true);
	    mdlScormService.saveOrUpdateMdlScormUser(mdlScormUser);
	} else {
	    log.error("finishActivity(): couldn't find MdlScormUser with id: " + mdlScormUser.getUserId()
		    + "and toolSessionID: " + toolSessionID);
	}

	ToolSessionManager sessionMgrService = MdlScormServiceProxy.getMdlScormSessionManager(getServlet()
		.getServletContext());

	String nextActivityUrl;
	try {
	    nextActivityUrl = sessionMgrService.leaveToolSession(toolSessionID, mdlScormUser.getUserId());
	    response.sendRedirect(nextActivityUrl);
	} catch (DataMissingException e) {
	    throw new MdlScormException(e);

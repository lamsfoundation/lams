...
public class AuthoringAction extends LamsDispatchAction {
    ...
    private static final String TOOL_APP_URL = Configuration.get(ConfigurationKeys.SERVER_URL) + "/tool/"
	    + MdlChatConstants.TOOL_SIGNATURE + "/";
	...
	// retrieving MdlChat with given toolContentID
	...
	if (mdlChat == null) {
	    mdlChat = mdlChatService.copyDefaultContent(toolContentID);
	...
	try {
		...
		responseUrl += "&id=" + mdlChat.getExtToolContentId().toString();
	    } else {
		responseUrl += "&add=Chat";
	    }

	    if (mdlChat.getExtCourseId() != null) {
		responseUrl += "&course=" + mdlChat.getExtCourseId();
	    } else {
		responseUrl += "&course=" + courseFromCSV;
	    }

	    log.debug("Sending to moodle Chat edit page: " + responseUrl);
	...
	// get mdlChat content.
	MdlChat mdlChat = mdlChatService.getMdlChatByContentId(toolContentID);
	mdlChat.setExtToolContentId(externalToolContentID);
	mdlChat.setUpdateDate(new Date());
	mdlChat.setDefineLater(false);
	mdlChatService.saveOrUpdateMdlChat(mdlChat);

	String redirectString = Configuration.get(ConfigurationKeys.SERVER_URL) + "/tool/"
		+ MdlChatConstants.TOOL_SIGNATURE + "/clearsession.do" + "?action=confirm&mode=author" + "&signature="
		+ MdlChatConstants.TOOL_SIGNATURE + "&toolContentID=" + toolContentID.toString() + "&defineLater=no"
		+ "&customiseSessionID=" + "&contentFolderID=0";

	log.debug("Manual redirect for MdlChat to: " + redirectString);

	try {
	    response.sendRedirect(redirectString);
	} catch (Exception e) {
	    log.error("Could not redirect to clear session action for MdlChat", e);
	}

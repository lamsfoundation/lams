...
public class LearningAction extends LamsDispatchAction {
	...
	if (mdlChatSession == null) {
	    throw new MdlChatException("Cannot retreive session with toolSessionID: " + toolSessionID);
	}
	...
	if (mdlChat.getExtToolContentId() != null) {
		...
		responseUrl += "&id=" + mdlChatSession.getExtSessionId() + "&returnUrl=" + returnUrl + "&dest="
			+ encodedMoodleRelativePath + "&is_learner=1" + "&isFinished="
			+ mdlChatUser.isFinishedActivity();
	...
	MdlChatUser mdlChatUser = mdlChatService.getUserByUserIdAndSessionId(new Long(user.getUserID().intValue()),
		toolSessionId);

	if (mdlChatUser == null) {
	    MdlChatSession mdlChatSession = mdlChatService.getSessionBySessionId(toolSessionId);
	    mdlChatUser = mdlChatService.createMdlChatUser(user, mdlChatSession);
	}
	...
	if (mdlChatUser != null) {
	    ...
	    log.error("finishActivity(): couldn't find MdlChatUser with id: " + mdlChatUser.getUserId()
		    + "and toolSessionID: " + toolSessionID);
	}

	ToolSessionManager sessionMgrService = MdlChatServiceProxy.getMdlChatSessionManager(getServlet()
		.getServletContext());

	String nextActivityUrl;
	try {
	    nextActivityUrl = sessionMgrService.leaveToolSession(toolSessionID, mdlChatUser.getUserId());

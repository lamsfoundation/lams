...
import org.lamsfoundation.lams.tool.mdquiz.dao.IMdlQuizConfigItemDAO;
...
public class MdlQuizService implements ToolSessionManager, ToolAdapterContentManager, IMdlQuizService,
    ...
    private IMdlQuizConfigItemDAO mdlQuizConfigItemDAO = null;
	...
	String cloneServletUrl = mdlQuizConfigItemDAO.getConfigItemByKey(MdlQuizConfigItem.KEY_EXTERNAL_TOOL_SERVLET)
		.getConfigValue();
	...
	try {
	    String outputServletUrl = mdlQuizConfigItemDAO.getConfigItemByKey(
		    MdlQuizConfigItem.KEY_EXTERNAL_TOOL_SERVLET).getConfigValue();
	...
	if (mdlQuiz.getExtToolContentId() == null) {
		...
		String exportServletUrl = mdlQuizConfigItemDAO.getConfigItemByKey(
			MdlQuizConfigItem.KEY_EXTERNAL_TOOL_SERVLET).getConfigValue();
	...
	IToolVO tool = toolService.getToolBySignature(MdlQuizConstants.TOOL_SIGNATURE);
	...
	if (extExportFile.exists()) {
		...
		String importServletUrl = mdlQuizConfigItemDAO.getConfigItemByKey(
			MdlQuizConfigItem.KEY_EXTERNAL_TOOL_SERVLET).getConfigValue();
	...
	/*
	Long defaultContentID = getDefaultContentIdBySignature(MdlQuizConstants.TOOL_SIGNATURE);
	MdlQuiz defaultContent = getMdlQuizByContentId(defaultContentID);
	...
	*/
	...
	return mdlQuizConfigItemDAO.getConfigItemByKey(key);
    }

    public void saveOrUpdateMdlQuizConfigItem(MdlQuizConfigItem item) {
	mdlQuizConfigItemDAO.saveOrUpdate(item);
    }
    ...
    public IMdlQuizConfigItemDAO getMdlQuizConfigItemDAO() {
	return mdlQuizConfigItemDAO;
    }

    public void setMdlQuizConfigItemDAO(IMdlQuizConfigItemDAO mdlQuizConfigItemDAO) {
	this.mdlQuizConfigItemDAO = mdlQuizConfigItemDAO;
    }

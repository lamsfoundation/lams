...
var padeditbar = (function()
{
  ...
  function registerDefaultCommands(toolbar) {
    ...
    toolbar.registerCommand("showTimeSlider", function () {
      // LAMS fix for returning to pad with same settings
      localStorage.setItem("etherpadTimesliderReturnUrl", document.location.href);
    ...
    toolbar.registerCommand('timeslider_returnToPad', function(cmd) {
      // LAMS fix for returning to pad with same settings
      let returnUrl = localStorage.getItem("etherpadTimesliderReturnUrl");
      if (returnUrl) {
    	  document.location.href = returnUrl;
      } else if( document.referrer.length > 0 && document.referrer.substring(document.referrer.lastIndexOf("/")-1, document.referrer.lastIndexOf("/")) === "p") {

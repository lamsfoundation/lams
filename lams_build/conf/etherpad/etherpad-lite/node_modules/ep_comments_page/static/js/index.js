...
EpComments.prototype.init = function () {
  ...
  // LAMS modified this logic so right comments bar is hidden by default
...
EpComments.prototype.collectCommentReplies = function (callback) {
  $.each(this.commentReplies, (replyId, reply) => {
    ...
    if (commentId) {
	  ...
	  // LAMS: fix unnecessary escaping broken by
	  // https://github.com/ether/ep_comments_page/issues/175
	  content[0].innerHTML = content[0].innerHTML.replaceAll('&amp;nbsp;', ' ');
...
EpComments.prototype.insertComment = function (commentId, comment, index) {
  ...
  // LAMS: fix unnecessary escaping broken by
  // https://github.com/ether/ep_comments_page/issues/175
  content[0].innerHTML = content[0].innerHTML.replaceAll('%20', ' ').replaceAll('&amp;nbsp;', ' ').replaceAll('%0A', ' ');
...
EpComments.prototype.displayNewCommentForm = function () {
  ...
  ace.callWithAce((ace) => {
    ...
    rep.hasCommentsOnSelection = ace.ace_hasCommentOnSelection();
  ...
  //LAMS modification: prevent comments from being overwritten
  if (rep.hasCommentsOnSelection) {
	$.gritter.add({
	  text: 'This text already has a comment. Reply on the existing comment instead.'
	});
	return;
  }

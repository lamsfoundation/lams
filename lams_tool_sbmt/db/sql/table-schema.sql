...
create table tl_lasbmt11_content (content_id bigint not null, title varchar(64) not null, instruction text, define_later bit not null, run_offline bit not null, offline_instruction text, online_instruction text, content_in_use bit, lock_on_finished bit, reflect_instructions varchar(255), reflect_on_activity bit, limit_upload bit, limit_upload_number integer, created datetime, created_by bigint, updated datetime, primary key (content_id));
...
create table tl_lasbmt11_report (report_id bigint not null auto_increment, comments varchar(250), marks bigint, date_marks_released datetime, primary key (report_id));

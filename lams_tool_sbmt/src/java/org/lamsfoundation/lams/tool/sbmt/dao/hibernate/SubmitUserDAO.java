...
public class SubmitUserDAO extends BaseDAO implements ISubmitUserDAO {
	...
	public List<Object[]> getUsersForTablesorter(final Long sessionId, int page, int size, int sorting, String searchString,
	   ...
	   // when sorting by number marked, keep the "can be marked" ones together, to make it easier for marking time
		...
		notebookEntryStrings = coreNotebookService.getNotebookEntrySQLStrings(sessionId.toString(),SbmtConstants.TOOL_SIGNATURE,"user.user_id" );
	    }
	    ...
	    query.addEntity("user", SubmitUser.class)
	    	.addScalar("numFiles", Hibernate.INTEGER)
	    	.addScalar("numFilesMarked", Hibernate.INTEGER)
	...
	public int getCountUsersBySession(final Long sessionId, String searchString) {
	    ...
	    StringBuilder queryText = new StringBuilder("SELECT count(*) FROM tl_lasbmt11_user user WHERE user.session_id = :sessionId ");
	    buildNameSearch(searchString, queryText);
	...
	public List<StatisticDTO> getStatisticsBySession(final Long contentId) {
		...
		.addScalar("totalUploadedFiles", Hibernate.INTEGER)
		.addScalar("markedCount", Hibernate.INTEGER)

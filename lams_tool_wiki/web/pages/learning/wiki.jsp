...
<div id="content"><!-- Display the advanced option warnings -->
<div class="info" id="messageDiv" style="display: none;"><c:if
	test="${wikiDTO.lockOnFinish and mode == 'learner'}">
	<p><c:choose>
...
</c:if> <c:if test="${not (wikiDTO.lockOnFinish and finishedActivity) }">
	<c:choose>
		<c:when test="${currentWikiPage.editable}">
			<c:if test="${not minEditsReached}">
				<p><fmt:message key="message.minumumEditsNotReached">
					<fmt:param>
...
&nbsp; <html:form action="/learning" method="post"
	styleId="learningForm" enctype="multipart/form-data">
	...
	<c:set var="editableFlag"
		value="${currentWikiPage.editable and not maxEditsReached}" />
	<c:set var="addFlag"
		value="${wikiDTO.allowLearnerCreatePages and not maxEditsReached}" />
	...
	<div id="buttons" style="float: right; width: 70%;"><c:if
		test="${contentEditable}">
		<c:if test="${editableFlag or addFlag}">
			<a href="javascript:changeDiv('view');" class="button"><fmt:message
				key="label.wiki.view"></fmt:message></a>
		</c:if>
		<c:if test="${editableFlag}">
			<a href="javascript:changeDiv('edit');" class="button"><fmt:message
				key="label.wiki.edit"></fmt:message></a>
		</c:if>
		<c:if test="${addFlag}">
			<a href="javascript:cancelAdd();changeDiv('add');" class="button"><fmt:message
				key="label.wiki.add"></fmt:message></a>
		</c:if>
		<c:if test="${editableFlag}">
			<a href="javascript:changeDiv('history');" class="button"><fmt:message
				key="label.wiki.history"></fmt:message></a>
		</c:if>
		<c:if
			test="${currentWikiPage.title != mainWikiPage.title && editableFlag}">
			<a
	...
	<table cellpadding="0">
		...
		<tr>
			<td align="right"><br />
			<a href="javascript:doEditOrAdd('editPage');" class="button"><fmt:message
				key="label.wiki.savechanges"></fmt:message></a> <a
	...
	<table cellpadding="0">
		...
		<tr>
			...
			<lams:FCKEditor id="newPageWikiBody" value="" height="400px"
				contentFolderID="${contentFolderID}" toolbarSet="Custom-Learner"></lams:FCKEditor>
			</td>
		</tr>
		<tr>
			<td align="right"><br />
			<a href="javascript:doEditOrAdd('addPage');" class="button"><fmt:message
				key="label.wiki.savechanges"></fmt:message></a> <a
				href="javascript:cancelAdd();changeDiv('view');" class="button"><fmt:message
				key="button.cancel"></fmt:message></a></td>
		</tr>
	</table>
	</div>
	...
	<!-- 
		...
		-->
</html:form> <c:if test="${mode == 'learner' || mode == 'author'}">

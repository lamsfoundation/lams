...
public class DimdimService implements ToolSessionManager, ToolContentManager,
	...
	public void removeToolSession(Long toolSessionId)
			throws DataMissingException, ToolException {
		dimdimSessionDAO.deleteByProperty(DimdimSession.class, "sessionId",
				toolSessionId);
	...
	public void copyToolContent(Long fromContentId, Long toContentId)
		...
		Dimdim toContent = Dimdim.newInstance(fromContent, toContentId,
				dimdimToolContentHandler);
	...
	public void exportToolContent(Long toolContentId, String rootPath)
		...
		if (dimdim == null)
			throw new DataMissingException(
					"Unable to find default content for the dimdim tool");
		...
		try {
			exportContentService.registerFileClassForExport(
					DimdimAttachment.class.getName(), "fileUuid",
					"fileVersionId");
			exportContentService.exportToolContent(toolContentId, dimdim,
					dimdimToolContentHandler, rootPath);
	...
	public void importToolContent(Long toolContentId, Integer newUserUid,
		...
		try {
			...
			Object toolPOJO = exportContentService.importToolContent(
					toolContentPath, dimdimToolContentHandler, fromVersion,
					toVersion);
			if (!(toolPOJO instanceof Dimdim))
				throw new ImportToolContentException(
						"Import Dimdim tool content failed. Deserialized object is "
								+ toolPOJO);
	...
	public Dimdim copyDefaultContent(Long newContentID) {
		...
		if (newContentID == null) {
			String error = "Cannot copy the Dimdim tools default content: + "
		...
		newContent = Dimdim.newInstance(defaultContent, newContentID,
				dimdimToolContentHandler);
	...
	public Dimdim getDimdimByContentId(Long toolContentID) {
		List<Dimdim> list = dimdimDAO.findByProperty(Dimdim.class,
				"toolContentId", toolContentID);
		if (list.isEmpty()) {
			return null;
		} else {
	...
	public DimdimSession getSessionBySessionId(Long toolSessionId) {
		List<DimdimSession> list = dimdimSessionDAO.findByProperty(
				DimdimSession.class, "sessionId", toolSessionId);
		if (list.isEmpty()) {
			return null;
		} else {
			return list.get(0);
	...
	public DimdimUser getUserByUID(Long uid) {
		List<DimdimUser> list = dimdimUserDAO.findByProperty(DimdimUser.class,
				"uid", uid);
		if (list.isEmpty()) {
			return null;
		} else {
	...
	public DimdimAttachment uploadFileToContent(Long toolContentId,
		...
		DimdimAttachment attachment = new DimdimAttachment(
	...
	private NodeKey processFile(FormFile file, String type) {
		...
		if (file != null && !StringUtils.isEmpty(file.getFileName())) {
			...
			try {
				node = getDimdimToolContentHandler().uploadFile(

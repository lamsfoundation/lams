...
CREATE INDEX content_id ON tl_lachat11_chat (tool_content_id ASC);
...
CREATE TABLE tl_lachat11_session (
     ...
     , PRIMARY KEY (uid)
     , INDEX (chat_uid)
     , CONSTRAINT FK_SESSION FOREIGN KEY (chat_uid)
                  REFERENCES tl_lachat11_chat (uid) ON DELETE NO ACTION ON UPDATE NO ACTION
)TYPE=InnoDB;
...
CREATE TABLE tl_lachat11_attachment (
     ...
     , PRIMARY KEY (uid)
     , INDEX (chat_uid)
     , CONSTRAINT FK_ATTACHMENT FOREIGN KEY (chat_uid)
                  REFERENCES tl_lachat11_chat (uid) ON DELETE NO ACTION ON UPDATE NO ACTION
)TYPE=InnoDB;

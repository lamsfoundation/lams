...
public class LearningAction extends DispatchAction {
	...
	String meetingKey = BbbUtil.getMeetingKey(user.getBbbSession().getSessionId());
	org.lamsfoundation.lams.usermanagement.dto.UserDTO lamsUserDTO = (org.lamsfoundation.lams.usermanagement.dto.UserDTO) SessionManager
		.getSession().getAttribute(AttributeNames.USER);
	...
	// Start a new bbb web meeting

	String meetingKey = BbbUtil.getMeetingKey(session.getSessionId());
	String returnURL = BbbUtil.getReturnURL(request);

/*	String meetingURL = bbbService.startConferenceURL(lamsUserDTO, meetingKey, returnURL);

...
public class RSA_SHA1 extends OAuthSignatureMethod {
	...
	try {
	    return verify(OAuthSignatureMethod.decodeBase64(signature), baseString.getBytes(OAuth.ENCODING));
	} catch (UnsupportedEncodingException e) {
	    throw new OAuthException(e);
	} catch (GeneralSecurityException e) {
	    throw new OAuthException(e);
	...
	if (privateKey == null) {
	    throw new IllegalStateException("need to set private key with " + "OAuthConsumer.setProperty when "
	...
	if (publicKey == null) {
	    throw new IllegalStateException("need to set public key with " + " OAuthConsumer.setProperty when "
	...
	if (privateKeyObject instanceof PrivateKey) {
		...
		privateKey = getPrivateKeyFromDer(OAuthSignatureMethod.decodeBase64((String) privateKeyObject));
	    }
	...
	if (publicKeyObject instanceof PublicKey) {
		...
		publicKey = getPublicKeyFromDer(OAuthSignatureMethod.decodeBase64((String) publicKeyObject));
	    }
	    ...
	    throw new IllegalArgumentException("Public key or certificate set through " + source + " must be of "

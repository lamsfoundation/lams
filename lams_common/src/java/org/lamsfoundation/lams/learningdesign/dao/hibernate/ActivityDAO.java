...
public class ActivityDAO extends LAMSBaseDAO implements IActivityDAO {
	...
	private static final String FIND_BY_PARENT = "from " + ActivityDAO.TABLENAME + " in class " + Activity.class.getName()
	...
	private static final String FIND_GROUPINGACTIVITY_TYPE_BY_LEARNING_DESIGN_ID = "from " + ActivityDAO.TABLENAME + " in class "
			+ GroupingActivity.class.getName() + " where learning_design_id=?";
	private static final String FIND_BY_UI_ID = "from " + ActivityDAO.TABLENAME + " in class " + Activity.class.getName()
			+ " where activity_ui_id=?" + " AND " + " learning_design_id=?";
	private static final String FIND_BY_LIBRARY_ID = "from " + ActivityDAO.TABLENAME + " in class " + Activity.class.getName()
	...
	private Activity getNonCGLibActivity(Activity act) {
		if (act != null) {
			...
			throw new DataRetrievalFailureException(
					"Unable to get activity as the activity type is unknown or missing. Activity type is " + activityType);
		}
		return null;
	}

		...
		try {
			...
			for (Integer orgId : orgIds) {
				...
				queryStr = "select distinct ug.user_id "
						...
						+ "where l.class_grouping_id=gg.grouping_id "
						+ "and gg.staff_group_id=g.group_id "
				...
				queryStr = "select distinct uo.user_id "
						+ "from lams_user_organisation uo, lams_ext_user_userid_map eu "
						+ "where uo.user_id=eu.user_id "
						+ "and uo.organisation_id=?";
				query = conn.prepareStatement(queryStr);
				query.setInt(1, orgId);
				if (query.execute()) {
					ResultSet rs = query.getResultSet();
				...
				for (Integer userId : userIds) {
					if (!staffIds.contains(userId)) {
						...
						// given org id
						queryStr = "delete uor.* "
								+ "from lams_user_organisation_role uor, lams_user_organisation uo "
								+ "where uo.user_id=? "
						...
						log.info("Deleted " + deleted
								+ " non-learner roles for user id " + userId
								+ " in org id " + orgId);
					}
				}
			}

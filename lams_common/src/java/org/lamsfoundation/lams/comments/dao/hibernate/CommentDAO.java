...
public class CommentDAO extends HibernateDaoSupport implements ICommentDAO {
	...
	query.addEntity("comment", Comment.class).addScalar("likes_total", Hibernate.INTEGER)
		.addScalar("user_vote", Hibernate.INTEGER).setLong("userId", userId != null ? userId : 0)
	...
	if (previousThreadMessageId == null || previousThreadMessageId == 0L) {
	    topThreadObjects = getSession().createSQLQuery(SQL_QUERY_FIND_FIRST_THREAD_TOP_BY_LIKES)
		    .addEntity("comment", Comment.class).addScalar("likes_total", Hibernate.INTEGER)
		    .addScalar("user_vote", Hibernate.INTEGER).setLong("rootUid", rootTopicId)

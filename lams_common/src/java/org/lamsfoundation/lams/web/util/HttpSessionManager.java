...
/**
 * It is used to manage HttpSession according to lams configuration (in lams.xml file). There are 2 configuration items
 * related to it. One is AllowMultipleLogin, the other is UserInactiveTimeout
 * <p>
 * When running with the test harness, need to set a configuration parameter allowMultipleLogins=true This disables the
 * invalidation of older duplicated sessions - so that the poll and application test threads will run together. It
 * shouldn't be run with this set to true in production as not all the sessions are tracked properly - only one session
 * is tracked for each user.
 * <p>
 * Why isn't this class a servlet rather than a singleton? Because it doesn't directly serve data back to a webpage -
 * its primary function is to return data to the controller. Yet it is in the "web" part of the system, so there is some
 * argument for it being a servlet. An advantage of it being a servlet would be that we could probably make it directly
 * a listener (rather than having a listener which calls this class), and we wouldn't need the test servlet
 * ReportHttpSessionDataServlet to dump the current status to a webpage.
 * 
 * @author Fei Yang
 * 
 */
public class HttpSessionManager {
    ...
    private Map<String, HttpSession> httpSessionsByLogin = null; // allows checking for two sessions
    private int timeout;//in seconds
    private boolean allowMultipleLogin; // must only be true for the test harness.

    private HttpSessionManager() {
	httpSessionsByLogin = Collections.synchronizedMap(new HashMap<String, HttpSession>());
	...
	if (HttpSessionManager.instance == null) {
	    HttpSessionManager.instance = new HttpSessionManager();
	}
	...
	return httpSessionsByLogin.get(login);
    }
	...
	if (!allowMultipleLogin) {
	    ...
	    HttpSession oldSession = httpSessionsByLogin.get(login);
	    if (oldSession != null) {
	...
	Enumeration<String> enu = oldSession.getAttributeNames();
	...
	return HttpSessionManager.context;
    }

    public void setServletContext(ServletContext context) {
	if (HttpSessionManager.context == null) {

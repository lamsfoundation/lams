 ...
 */
public class NotifyActionJSON extends JSONObject {
    ...
    public NotifyActionJSON(Long requestId, Long nodeId, int type, Long childNodeId, String text, String color, String creator) throws JSONException {
	super();
	this.put(REQUEST_ID_KEY, requestId);
	this.put(NODE_ID_KEY, nodeId);
	this.put(TYPE_KEY, type);
	this.put(CHILD_NODE_ID_KEY, childNodeId);
	this.putOpt(TITLE_KEY, text);
	this.putOpt(BACKGROUND_COLOR, color);
	this.putOpt(CREATOR, creator);
    }

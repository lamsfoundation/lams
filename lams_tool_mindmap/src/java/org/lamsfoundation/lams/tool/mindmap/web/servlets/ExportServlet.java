...
public class ExportServlet extends AbstractExportPortfolioServlet {
	...
	try {
		...
		writeResponseToFile(basePath + "/pages/export/exportPortfolioSinglemode.jsp", directoryName, FILENAME,
			cookies);
		...
		if (mindmap.isMultiUserMode()) {
			...
			writeResponseToFile(basePath + "/pages/export/exportPortfolioMultimode.jsp", directoryName,
				filename, cookies);
		    }
		    request.getSession().setAttribute("mindmapDTO", mindmapDTO);

		    writeResponseToFile(basePath + "/pages/export/exportPortfolioMultimodeLinks.jsp", directoryName,
			    FILENAME, cookies);
			...
			for (Iterator userIterator = userDTOs.iterator(); userIterator.hasNext();) {
			    ...
			    String filename = mindmapUser.getFirstName() + "_" + mindmapUser.getLastName() + "_"
				    + mindmapUser.getUid() + ".html";

			    doTeacherSingleModeExport(request, response, directoryName, cookies, mindmap, mindmapUser);
			    writeResponseToFile(basePath + "/pages/export/exportPortfolioSinglemode.jsp", directoryName,
				    filename, cookies);
			}
		    }
		    ...
		    writeResponseToFile(basePath + "/pages/export/exportPortfolioSinglemodeLinks.jsp", directoryName,
			    FILENAME, cookies);
    ...
    private void doTeacherSingleModeExport(HttpServletRequest request, HttpServletResponse response,
	    String directoryName, Cookie[] cookies, Mindmap mindmap, MindmapUser mindmapUser) throws MindmapException {

	logger.debug("doTeacherSingleModeExport: toolContentID: " + toolContentID);

	String filename = mindmapUser.getFirstName() + "_" + mindmapUser.getLastName() + "_" + mindmapUser.getUid();
    ...
    private void doTeacherMultiModeExport(HttpServletRequest request, HttpServletResponse response,
	    String directoryName, Cookie[] cookies, Mindmap mindmap, MindmapSessionDTO mindmapSessionDTO)
	...
	List mindmapNodeList = mindmapService.getAuthorRootNodeByMindmapSession(mindmap.getUid(),
		mindmapSessionDTO.getSessionID());

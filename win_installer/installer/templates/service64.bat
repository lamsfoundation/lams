...
echo Usage: service64 install^|uninstall^|start^|stop^|restart^|signal
goto cmdEnd
...
:cmdInstall
jbosssvc.x64.exe -imwdc %SVCNAME% "%DIRNAME%" "%SVCDISP%" "%SVCDESC%" service64.bat
if not errorlevel 0 goto errExplain
echo Service %SVCDISP% installed
goto cmdEnd

:cmdUninstall
jbosssvc.x64.exe -u %SVCNAME%
if not errorlevel 0 goto errExplain
...
jbosssvc.x64.exe -s 1
if exist ".r.lock" goto waitRun
echo Y > .r.lock
jbosssvc.x64.exe -p 1 "Restarting %SVCDISP%" >> run.log
call run.bat < .r.lock >> run.log 2>&1
jbosssvc.x64.exe -p 1 "Shutdown %SVCDISP% service" >> run.log
del .r.lock
goto cmdEnd

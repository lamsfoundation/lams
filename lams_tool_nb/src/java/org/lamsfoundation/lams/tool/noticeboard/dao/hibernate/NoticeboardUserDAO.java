...
public class NoticeboardUserDAO extends HibernateDaoSupport implements INoticeboardUserDAO {
	...
	List usersReturned = getSession().createQuery(FIND_NB_USER_BY_SESSION).setLong(0, userId.longValue())
		.setLong(1, sessionId.longValue()).list();
	...
	if (userId != null) {
	    ...
	    List list = getSession().createQuery(FIND_NB_USER).setLong(0, userId.longValue()).list();
	...
	return (getHibernateTemplate().findByNamedParam(COUNT_USERS_IN_SESSION, "nbSession", nbSession)).size();
    }

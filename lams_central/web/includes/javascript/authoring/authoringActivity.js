	...
	ActivityLib = {
		...
		dropActivity : function(activity, x, y) {
			if (!(activity instanceof ActivityDefs.OptionalActivity || activity instanceof ActivityDefs.FloatingActivity)) {
				...
				} else if (activity instanceof ActivityDefs.ToolActivity){
					// prevent activity overlapping
					let activityBox = activity.items.getBBox();
					$.each(layout.activities, function(){
						if (this.uiid !== activity.uiid
							&& (this instanceof ActivityDefs.ToolActivity  || this instanceof ActivityDefs.GroupingActivity)) {
							let thisBox = this.items.getBBox();
							if (Snap.path.isPointInsideBBox(thisBox, activityBox.x, activityBox.y)) {
								activity.draw(activityBox.x, thisBox.y2 + 15);
								return false;
							}
						}
					});

		...
		<td class="arrows" width="40px">
			...
			<c:if test="${optionNumber > 1}">
 				<lams:Arrow state="up" id="${qnon}UpButton" title="<fmt:message key='hint.option.up'/>" 
 					onclick="javascript:swapOptions(${questionNumber}, ${optionNumber}, ${optionNumber-1}, 'divassmcq${questionNumber}options', true)"/>
 			</c:if>
			...
			<lams:Arrow state="down" id="${qnon}DownButton" title="<fmt:message key='hint.option.down'/>" 
				onclick="javascript:swapOptions(${questionNumber}, ${optionNumber}, ${optionNumber+1}, 'divassmcq${questionNumber}options', true)" display="${display}"/>
 		</td>
		<td width="20px" align="center">
			<i class="fa fa-times" id="${qnon}DeleteButton" title="<fmt:message key='hint.option.delete'/>" onclick="removeOption(${questionNumber}, ${optionNumber}, 'divassmcq${questionNumber}options', true)">
		</td>
		</tr>
		</table>
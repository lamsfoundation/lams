...
<!--
		...
		var authoring_size = "<lams:Configuration key='AuthoringScreenSize'/>";
		
		if(!str_regex.test(authoring_size))
			authoring_size = "796x570";			// default size
		...
		var monitor_size = "<lams:Configuration key='MonitorScreenSize'/>";
		if(!str_regex.test(monitor_size))
			monitor_size = "796x575";			// default size
		...
		var learner_size = "<lams:Configuration key='LearnerScreenSize'/>";
		if(!str_regex.test(learner_size))
			learner_size = "796x570"; 			// default size
		...
		var sys_admin_size = "<lams:Configuration key='AdminScreenSize'/>";
		if(!str_regex.test(sys_admin_size))
			sys_admin_size = "850x650";			// default size

...
<body class="stripes">
	...
	<h2>
		<fmt:message key="label.create.lesson"/>
	</h2>
    ...
    <form name="lessonForm" id="lesson-form" action="lti.do?method=startLesson" method="post" onSubmit="return confirmSubmit();">
    	...
    	<table id="title-table">
			<tr>
				...
				<td>
					<input type="text" name="title" size="70" id="title" value="${title}"/>
				<td>
			</tr>
			...
			<tr>
				...
				<td>
					<textarea name="desc" cols="70" rows="3" id="desc">${desc}</textarea>
				</td>
			</tr>
		</table>
            
        <h4>
        	<fmt:message key="label.choose.sequence"/>
		</h4>
		
		<%-- Preview and Author Buttons --%>			    
		<div class="float-right">
			<input type="button" id="preview-button" onclick="openPreview(); return false;" 
					class="button" value="<fmt:message key="authoring.fla.preview.lesson.default.title" />" >
						
			<input type="button" onclick="openAuthor(true); return false;" 
					class="button" value="<fmt:message key="label.author.sequence" />" >
			    
			<input type="button" onclick="window.location.reload(); return false;" 
					class="button" value="<fmt:message key="authoring.fla.refresh.button" />" >
		</div>
		...
		<div class="space-bottom-top align-right">
			<input type="submit" class="button" value="<fmt:message key="authoring.fla.save.button" />">
		</div>
    </form>

				...
				<div class="float-end">
					<c:if test='${sessionMap.mode == "teacher" && not sessionMap.readOnly}'>
						...
						<c:choose>
							<c:when test="${hidden}">
								<!--  display a show link  -->
								<c:set var="hideURL">
									<lams:LAMSURL />comments/hide.do?sessionMapID=${sessionMapID}&commentUid=${commentDto.comment.uid}&hideFlag=false
								</c:set>
								<button type="button" onclick="javascript:hideEntry(${commentDto.comment.uid}, '${hideURL}');" class="btn btn-sm btn-light comment">
							...
							<c:otherwise>
								<!--  display a hide link -->
								<c:set var="hideURL">
									<lams:LAMSURL />comments/hide.do?sessionMapID=${sessionMapID}&commentUid=${commentDto.comment.uid}&hideFlag=true
								</c:set>
								<button type="button" onclick="javascript:hideEntry(${commentDto.comment.uid}, '${hideURL}');" class="btn btn-sm btn-light comment">
					...
					<c:if test='${sessionMap.mode == "teacher" && not sessionMap.readOnly && commentDto.comment.commentLevel == 1}'>
						<c:set var="makesticky">
							<lams:LAMSURL />comments/makeSticky.do?sessionMapID=${sessionMapID}&commentUid=${commentDto.comment.uid}&create=${commentDto.comment.created.time}&sticky=${!commentDto.comment.sticky}
						</c:set>
						<button type="button" onclick="javascript:makeSticky(${commentDto.comment.uid}, '${makesticky}');" class="btn btn-sm btn-light comment">
					...
					<c:if test='${((sessionMap.mode == "teacher") || commentDto.author) && not sessionMap.readOnly }'>
              			&middot;
						<c:set var="edittopic">
							<lams:LAMSURL />comments/editTopic.do?sessionMapID=${sessionMapID}&commentUid=${commentDto.comment.uid}&create=${commentDto.comment.created.time}
						</c:set>
					...
					<c:if test='${not sessionMap.readOnly}'>
						...
						<c:set var="replytopic">
							<lams:LAMSURL />comments/newReplyTopic.do?sessionMapID=${sessionMapID}&parentUid=${commentDto.comment.uid}
						</c:set>

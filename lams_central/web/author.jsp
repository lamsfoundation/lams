...
<c:set var="authorurl_params">?loadFile=lams_authoring.swf&loadLibrary=lams_authoring_library.swf&userID=<lams:user property="userID"/>&serverURL=<lams:LAMSURL/>&build=<%=authoringClientVersion%>&version=<%=version%>&lang=<lams:user property="localeLanguage"/>&country=<lams:user property="localeCountry"/><c:if test="${not empty user_dir}">&direction=<c:out value="${user_dir}"/></c:if>&langDate=<%=languageDate%>&theme=<lams:user property="flashTheme"/>&actColour=<%= actColour %><c:if test="${not empty extlmsid}">&extlmsid=${extlmsid}</c:if><c:if test="${not empty customCSV}">&customCSV=${customCSV}</c:if><c:if test="${not empty requestSrc}">&requestSrc=${requestSrc}</c:if><c:if test="${not empty learningDesignID}">&learningDesignID=${learningDesignID}</c:if><c:if test="${not empty layout}">&layout=${layout}</c:if></c:set>

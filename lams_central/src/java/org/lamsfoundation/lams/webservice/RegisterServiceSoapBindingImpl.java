...
public class RegisterServiceSoapBindingImpl implements Register {
    ...
    private static IUserManagementService service = (IUserManagementService) WebApplicationContextUtils
	    .getRequiredWebApplicationContext(
		    ((HttpServlet) RegisterServiceSoapBindingImpl.context.getProperty(HTTPConstants.MC_HTTP_SERVLET))
			    .getServletContext())
	...
	try {
	    ExtServerOrgMap extServer = RegisterServiceSoapBindingImpl.integrationService.getExtServerOrgMap(serverId);
	    Authenticator.authenticate(extServer, datetime, hash);
	    if (RegisterServiceSoapBindingImpl.service.getUserByLogin(username) != null) {
		return false;
	    }
	    ExtUserUseridMap userMap = RegisterServiceSoapBindingImpl.integrationService
		    .getImplicitExtUserUseridMap(extServer, username, password, null, firstName, lastName, email);
	    return true;
	} catch (Exception e) {
	...
	try {
	    ...
	    org.setParentOrganisation(RegisterServiceSoapBindingImpl.service.getRootOrganisation());
	    org.setOrganisationType((OrganisationType) RegisterServiceSoapBindingImpl.service
		    .findById(OrganisationType.class, OrganisationType.COURSE_TYPE));
	    org.setOrganisationState((OrganisationState) RegisterServiceSoapBindingImpl.service
	    ...
	    User user = RegisterServiceSoapBindingImpl.service.getUserByLogin(owner);
	    RegisterServiceSoapBindingImpl.service.saveOrganisation(org, user.getUserId());
	...
	try {
	    ...
	    User user = RegisterServiceSoapBindingImpl.service.getUserByLogin(login);
	    Organisation org = (Organisation) RegisterServiceSoapBindingImpl.service.findById(Organisation.class,
		    organisationId);
	    ...
	    UserOrganisation uo = RegisterServiceSoapBindingImpl.service.getUserOrganisation(user.getUserId(),
		...
		RegisterServiceSoapBindingImpl.service.save(uo);
		Integer[] roles;
		if (asStaff) {
		...
		for (Integer roleId : roles) {
		    UserOrganisationRole uor = new UserOrganisationRole(uo,
			    (Role) RegisterServiceSoapBindingImpl.service.findById(Role.class, roleId));
		    RegisterServiceSoapBindingImpl.service.save(uor);
		    uo.addUserOrganisationRole(uor);
		}
		user.addUserOrganisation(uo);
		RegisterServiceSoapBindingImpl.service.save(user);

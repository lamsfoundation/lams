...
public class FlashCrashDumpServlet extends AbstractStoreWDDXPacketServlet {
	...
	if (user == null) {
	    ...
	    return new FlashMessage(MESSAGE_KEY, "User not logged in - unable to dump file.", FlashMessage.ERROR)
		    .serializeMessage();
	}
	...
	try {
	    ...
	    log.warn(
		    "FlashCrashDumpServlet: Unable to deserialize packet - invalid data. Just writing out packet " + id,
		    e);
	}

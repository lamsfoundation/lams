...
public interface IAuthoringService {
    ...
    /**
     * Returns a populated LearningDesign object corresponding to the given
     * learningDesignID
     * 
     * @param learningDesignID
     *            The learning_design_id of the design which has to be fetched
     * @return LearningDesign The populated LearningDesign object corresponding
     *         to the given learningDesignID
     */
    public LearningDesign getLearningDesign(Long learningDesignID);

    /**
     * Create a copy of learning design as per the requested learning design and
     * saves it in the given workspacefolder. Does not set the original
     * 
     * @param originalLearningDesign
     *            The source learning design id.
     * @param copyType
     *            purpose of copying the design. Can have one of the follwing
     *            values
     *            <ul>
     *            <li>LearningDesign.COPY_TYPE_NONE (for authoring enviornment)</li>
     *            <li>LearningDesign.COPY_TYPE_LESSON (for monitoring
     *            enviornment while creating a Lesson)</li>
     *            <li>LearningDesign.COPY_TYPE_PREVIEW (for previewing purposes)
     *            </li>
     *            </ul>
     * @param user
     *            The user who has sent this request(author/teacher)
     * @param setOriginalDesign
     *            If true, then sets the originalLearningDesign field in the new
     *            design
     * @param custom
    ...
    /**
     * Create a copy of learning design as per the requested learning design and
     * saves it in the given workspacefolder. Designed to be called when user
     * tries to copy a learning design using the Flash interface. Does not set
     * the original learning design field, so it should not be used for creating
     * lesson learning designs.
     * 
     ...
     *            </ul>
     * @param userID
     *            The user_id of the user who has sent this
     *            request(author/teacher)
     * @param workspaceFolderID
     *            The workspacefolder where this copy of the design would be
     *            saved
     * @param setOriginalDesign
     *            If true, then sets the originalLearningDesign field in the new
     *            design
    ...
    /**
     * Insert a learning design into another learning design. This is a copy and
     * paste type of copy - it just dumps the contents (with modified activity
     * ui ids) in the main learning design. It doesn't wrap up the contents in a
     ...
     *            The design to be imported into originalLearningDesign.
     *            Required.
     * @param userId
     *            Current User. Required.
     * @param customCSV
     *            The custom CSV required to insert tool adapter tools, so their
     *            content can be copied in the external server
     * @param createNewLearningDesign
     *            If true, then a copy of the originalLearningDesign is made and
     *            the copy modified. If it is false, then the
     *            originalLearningDesign is modified. Required.
     * @param newDesignName
     *            New name for the design if a new design is being create.
     *            Optional.
     * @param workspaceFolderID
     *            The folder in which to put the new learning design if
     *            createNewLearningDesign = true. May be null if
     ...
     * 
     * @param learningDesignID
     *            The learning_design_id of the design whose WDDX packet is
     *            requested
    ...
    /**
     * This method returns a list of all available Learning Designs in WDDX
     * format.
     * 
    ...
    /**
     * Returns a list of LearningDesign's in WDDX format, belonging to the given
     * user
     * 
     * @param user
    ...
    /**
     * This method returns a list of all available system libraries in WDDX
     * format.
     * 
     ...
     * 
     * Typically, when a user clicks on an activity to edit the tool contnet, it
     * must have a tool content id passed to it. This method uses the
     ...
     * Returns a string representing the new tool content id in WDDX format.
     * 
     * The is called when the user copies and pastes a tool activity icon in
     * authoring. It should only be called on a ToolActivity - never a Gate or
     * Grouping or Complex activity.
     * 
     * @param toolContentID
    ...
    /**
     * Calls an appropriate tools to copy the content indicated by
     * toolContentIds. Batch version of String copyToolContent(Long
     * toolContentID). Returns a map containing the old and new ids, and this is
     * converted to a WDDX format in the calling servlet.
     * 
     * The is called when the user copies and pastes a complex activity icon in
     * authoring. Authoring calls this method to copy all the contained tool
     * activities' content in one go.
     * 
    ...
    /**
     * Get the available licenses. This will include our supported Creative
     * Common licenses and an "OTHER" license which may be used for user entered
     * license details. The picture url supplied should be a full URL i.e. if it
     * was a relative URL in the database, it should have been converted to a
     * complete server URL (starting http://) before sending to the client.
     * 
     * @return Vector of LicenseDTO objects. It is a Vector to ensure
     *         compatibility with WDDX
     */
     ...
     * 
     * @param learningDesignID
     *            The learning_design_id of the design for which editing has
     *            finished.
     * @param userID

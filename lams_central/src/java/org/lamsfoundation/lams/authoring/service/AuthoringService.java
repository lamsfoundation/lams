...
public class AuthoringService implements IAuthoringService, BeanFactoryAware {
	...
	public String finishEditOnFly(Long learningDesignID, Integer userID,
		...
		if (design != null) { /*
			...
			if (design.getEditOverrideUser().equals(user)) {
				...
				ArrayList<Activity> activities = processor
	 ...
	 * @param design
	 */
	public void addSystemGateAfterActivity(ArrayList<Activity> activities,
			LearningDesign design) {
		GateActivity gate = null;
...

	{
		...
		updateDesignCompetences(originalLearningDesign, newLearningDesign,
				false);
		HashMap<Integer, Activity> newActivities = updateDesignActivities(
	 ...
	 */
	private HashMap<Integer, Activity> updateDesignActivities(
	...
	private void processActivity(Activity activity,
			...
			Map<Integer, Grouping> newGroupings, Activity parentActivity,
	...
	public void updateDesignCompetences(LearningDesign originalLearningDesign,
		...
		Set<Competence> oldCompetences = originalLearningDesign
				.getCompetences();
	...
	public void insertCompetenceMappings(Set<Competence> oldCompetences,
			Set<Competence> newCompetences,
	...
	public Competence getCompetenceFromSet(Set<Competence> competences,
			String title) {
		Competence ret = null;
	 ...
	 *            The copy of the originalLearningDesign
	 */
	public void updateCompetenceMappings(Set<Competence> newCompetences,
			HashMap<Integer, Activity> newActivities) {
		for (Integer activityKey : newActivities.keySet()) {
			Activity activity = newActivities.get(activityKey);
			if (activity.isToolActivity()) {
	...
	private void updateEvaluations(HashMap<Integer, Activity> newActivities) {
		...
		for (Integer key : newActivities.keySet()) {
			...
			if (activity.isToolActivity()) {
				Set<ActivityEvaluation> newActivityEvaluations = ((ToolActivity) activity)
	...
	private Activity getActivityCopy(final Activity activity,
			Map<Integer, Grouping> newGroupings, int uiidOffset) {
	 ...
	 * @throws Exception
	 */
	public Vector<ValidationErrorDTO> validateLearningDesign(
	...
	public Vector<AuthoringActivityDTO> getToolActivities(
	 ...
	 * <code>getAllLearningDesignDetails</code> to pack the required
	...
	public String copyMultipleToolContent(Integer userId,
			List<Long> toolContentIds, String customCSV) {
	...
	public String getUniqueNameForLearningDesign(String originalTitle,
		...
		if (workspaceFolderId != null) {
			List<String> ldTitleList = learningDesignDAO

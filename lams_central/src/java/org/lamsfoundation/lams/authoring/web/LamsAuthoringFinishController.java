...
public abstract class LamsAuthoringFinishController {
	...
	if (StringUtils.equals(action, CONFIRM_ACTION)) {
	    ...
	    IToolVO tool = getToolService(applicationContext).getToolBySignature(signature);
    ...
    private ILamsToolService getToolService(ApplicationContext applicationContext) {
	return (ILamsToolService) applicationContext.getBean(AuthoringConstants.TOOL_SERVICE_BEAN_NAME);
    }

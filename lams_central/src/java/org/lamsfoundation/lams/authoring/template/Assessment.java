...
//    public static final short ASSESSMENT_QUESTION_TYPE_ORDERING = 7;
	...
	if (type == ASSESSMENT_QUESTION_TYPE_MULTIPLE_CHOICE) {
	    ...
	    json.put(RestTags.ANSWERS, answersJSON);
	} else {
	...
	if (questionText == null || questionText.length() == 0) {
	    errorMessages = addError(errorMessages,
		    TextUtil.getText(appBundle, formatter, "authoring.error.application.exercise.num", new Object[] { questionNumber }));
	}
	if (type == ASSESSMENT_QUESTION_TYPE_MULTIPLE_CHOICE) {
	    if (answers.size() == 0) {
		...
		if (!found100percent) {
		    errorMessages = addError(errorMessages,
			    TextUtil.getText(appBundle, formatter, "authoring.error.application.exercise.must.have.100.percent", new Object[] { questionNumber }));
		}
	    }
	}
	return errorMessages;

...
public class ObjectExtractor implements IObjectExtractor {
	...
	/*
	 * The newActivityMap is a local copy of all the current activities. This
	 * will include the "top level" activities and subactivities. It is used to
	 * "crossreference" activities as we go, without having to repull them from
	 * the database. The keys are the UIIDs of the activities, not the IDs. It
	 * is important that the values in this map are the Activity objects related
	 * to the Hibernate session as they are updated by the parseTransitions
	 * code.
	 */
	protected HashMap<Integer, Activity> newActivityMap = new HashMap<Integer, Activity>();
	/*
	 * Record the tool sessions and activities as we go for edit on fly. This is
	 * needed in case we delete any. Cannot get them at the end as Hibernate
	 * tries to store the activities before getting the tool sessions, and this
	 * fails due to a foriegn key error. Its the foriegn key error that we are
	 * trying to avoid!
	 */
	...
	// ToolSession
	/*
	 * Used for easy access (rather than going back to the db) and makes it
	 * easier to clean them up at the end - they can't be deleted easily via the
	 * cascades as we don't get a list of deleted entries sent back from the
	 * client! We would end up with mappings still attached to a sequence
	 * activity that shouldn't be there! Not done as a map as we sometimes will
	 * check via UIID, sometimes by ID
	 */
	...
	/*
	 * can't delete as we go as they are linked to other items and have no way
	 * of knowing from the packet which ones will need to be deleted, so start
	 * off assuming all need to be deleted and remove the ones we want to keep.
	 */

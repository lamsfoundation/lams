...
public class DacoUserDAOHibernate extends BaseDAOHibernate implements DacoUserDAO {
	...
	/** Will return List<[DacoUser, Integer (record count), String (notebook entry)], [DacoUser, Integer, String], ... , [DacoUser, Integer, String]>
	...
	*/
	public List<Object[]> getUsersForTablesorter(final Long sessionId, int page, int size, int sorting, String searchString, 
		...
		queryText.append(" JOIN tl_ladaco10_sessions sess on user.session_uid = sess.uid and sess.session_id = :sessionId");
		...
		SQLQuery query =  getSession().createSQLQuery(queryText.toString());
		    query.addEntity("user", DacoUser.class)
			.addScalar("record_count", Hibernate.INTEGER)

...
//		SpreadsheetForm existForm = (SpreadsheetForm) sessionMap.get(SpreadsheetConstants.ATTR_RESOURCE_FORM);
	 ...
	 * @return
	 */
	private ActionForward saveUserSpreadsheet(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
...
//		 get the existing reflection entry
	...
	private ActionForward submitReflection(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		NotebookEntry entry = service.getEntry(sessionId, CoreNotebookConstants.NOTEBOOK_TOOL, SpreadsheetConstants.TOOL_SIGNATURE, userId);

		if (entry == null) {
			// create new entry
			service.createNotebookEntry(sessionId, CoreNotebookConstants.NOTEBOOK_TOOL,	SpreadsheetConstants.TOOL_SIGNATURE, userId, refForm.getEntryText());
		} else {

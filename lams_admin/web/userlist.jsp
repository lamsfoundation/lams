			...
			<c:forEach var="userManageBean" items="${userManageForm.userManageBeans}">
				<tr>
					<td>
						...
						<c:if test="${not empty userManageBean.email and userManageBean.login ne userManageBean.email}">
							<br>
							<span class="text-muted small" title="<fmt:message key="admin.user.email"/>">
									<c:out value="${userManageBean.email}" />
								</span>
						</c:if>
					</td>
					<td>

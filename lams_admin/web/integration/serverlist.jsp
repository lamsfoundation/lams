	...
	<lams:Page type="admin" title="${title}">
		...
		<nav aria-label="breadcrumb">
		  <ol class="breadcrumb">
		    ...
		    <li class="breadcrumb-item active" aria-current="page"><fmt:message key="sysadmin.maintain.external.servers"/></li>
		  </ol>
		</nav>
			...
			<table class="table table-striped table-bordered">
				...
				<tr <c:if test="${server.disabled}">class="table-danger ${signupOrganisation.disabled}"</c:if> >
					<td>
						<a href="<lams:LAMSURL/>admin/extserver/edit.do?sid=${server.sid}" title="<fmt:message key="admin.edit" />" class="font-weight-bold">
						...
						<c:choose>
							...
							<c:otherwise>
								...
								<a class="btn btn-sm btn-outline-secondary py-0 px-1" style="font-size: 0.8em;" title="<fmt:message key="sysadmin.serverkey" />" href="javascript:alert('<fmt:message key="sysadmin.serverkey" />: <c:out value="${server.serverkey}" escapeXml="true"/>');">
					...
					<td class="text-center">
						<a style="display: inline-block;" class="btn btn-outline-primary btn-sm" id="edit_<c:out value='${server.serverid}' escapeXml="true"/>" href="<lams:LAMSURL/>admin/extserver/edit.do?sid=${server.sid}" title="<fmt:message key="admin.edit" />">
			...
			<div class="pull-right">
				...
				<input class="btn btn-primary btn-sm loffset5" name="addnewserver" type="button" value="<fmt:message key='sysadmin.server.add' />" onClick="javascript:document.location='edit.do'" />
			</div>
	</lams:Page>
</body>
</lams:html>

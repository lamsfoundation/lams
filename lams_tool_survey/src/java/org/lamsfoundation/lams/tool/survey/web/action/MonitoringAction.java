...
public class MonitoringAction extends Action {
	...
	private ActionForward summary(ActionMapping mapping, ActionForm form,
		...
		sessionMap.put(AttributeNames.PARAM_CONTENT_FOLDER_ID, WebUtil
				.readStrParam(request, AttributeNames.PARAM_CONTENT_FOLDER_ID));

		request.setAttribute("initialTabId", WebUtil.readLongParam(request,
				AttributeNames.PARAM_CURRENT_TAB, true));

		Long contentId = WebUtil.readLongParam(request,
				AttributeNames.PARAM_TOOL_CONTENT_ID);
		ISurveyService service = getSurveyService();

		// get summary
		SortedMap<SurveySession, List<AnswerDTO>> summary = service
		...
		// get statistic
		SortedMap<SurveySession, Integer> statis = service
				.getStatistic(contentId);

		// cache into sessionMap
		sessionMap.put(SurveyConstants.ATTR_SUMMARY_LIST, summary);
		sessionMap.put(SurveyConstants.ATTR_STATISTIC_LIST, statis);
		sessionMap.put(SurveyConstants.PAGE_EDITABLE, new Boolean(
				SurveyWebUtils.isSurveyEditable(survey)));
		sessionMap.put(SurveyConstants.ATTR_SURVEY, survey);
		sessionMap.put(AttributeNames.PARAM_TOOL_CONTENT_ID, contentId);
		sessionMap.put(SurveyConstants.ATTR_IS_GROUPED_ACTIVITY,
				service.isGroupedActivity(contentId));
		...
		if (submissionDeadline != null) {
			...
			Date tzSubmissionDeadline = DateUtil.convertToTimeZoneFromDefault(
					learnerTimeZone, submissionDeadline);
			MonitoringAction.log.info("Time:" + tzSubmissionDeadline.getTime());
			// store submission deadline to sessionMap
			sessionMap.put(SurveyConstants.ATTR_SUBMISSION_DEADLINE,
					tzSubmissionDeadline.getTime());
	...
	private ActionForward listAnswers(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) {
		Long sessionId = WebUtil.readLongParam(request,
				AttributeNames.PARAM_TOOL_SESSION_ID);
		Long questionUid = WebUtil.readLongParam(request,
				SurveyConstants.ATTR_QUESTION_UID);
	...
	private ActionForward getAnswersJSON(ActionMapping mapping,
		...
		if (sortByName != null)
			sorting = sortByName.equals(0) ? SurveyConstants.SORT_BY_NAME_ASC
					: SurveyConstants.SORT_BY_NAME_DESC;

		// return user list according to the given sessionID
		ISurveyService service = getSurveyService();
		SurveyQuestion question = service.getQuestion(questionUid);
		List<Object[]> users = service.getQuestionAnswersForTablesorter(
				sessionId, questionUid, page, size, sorting, searchString);

		JSONArray rows = new JSONArray();
		JSONObject responsedata = new JSONObject();
		responsedata.put("total_rows",
				service.getCountUsersBySession(sessionId, searchString));
		...
		for (Object[] userAndAnswers : users) {
			...
			responseRow.put(
					SurveyConstants.ATTR_USER_NAME,
					StringEscapeUtils.escapeHtml(user.getLastName() + " "
							+ user.getFirstName()));

			if (userAndAnswers.length > 1 && userAndAnswers[1] != null) {
				responseRow.put("choices", SurveyWebUtils
						.getChoiceList((String) userAndAnswers[1]));
			}
			if (userAndAnswers.length > 2 && userAndAnswers[2] != null) {
				...
				if (question.getType() == SurveyConstants.QUESTION_TYPE_TEXT_ENTRY) {
					...
					answer = StringEscapeUtils
							.escapeHtml((String) userAndAnswers[2]);
	...
	private ActionForward listReflections(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
	...
	private ActionForward getReflectionsJSON(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws JSONException, IOException {
		...
		// return user list according to the given sessionID
		ISurveyService service = getSurveyService();
		List<Object[]> users = service.getUserReflectionsForTablesorter(
				sessionId, page, size, sorting, searchString);
		...
		for (Object[] userAndReflection : users) {
			...
			if (userAndReflection.length > 1 && userAndReflection[1] != null) {
				String reflection = StringEscapeUtils
						.escapeHtml((String) userAndReflection[1]);
				responseRow.put(SurveyConstants.ATTR_REFLECTION,
						reflection.replaceAll("\n", "<br>"));
			}
	 ...
	 * @return
	 */
	private ActionForward exportSurvey(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) {
		Long toolSessionID = new Long(WebUtil.readLongParam(request,
				AttributeNames.PARAM_TOOL_SESSION_ID));
		...
		try {
			...
			Cell cell;
			int idx = 0;
			Set<Entry<SurveySession, SortedMap<SurveyQuestion, List<AnswerDTO>>>> entries = groupList
					.entrySet();
			for (Entry<SurveySession, SortedMap<SurveyQuestion, List<AnswerDTO>>> entry : entries) {
				SurveySession session = entry.getKey();
				SortedMap<SurveyQuestion, List<AnswerDTO>> map = entry
						.getValue();
				...
				cell.setCellValue(resource
						.getMessage(MonitoringAction.MSG_LABEL_SESSION_NAME));
				cell = row.createCell(1);
				cell.setCellValue(removeHTMLTags(session.getSessionName()));

				// begin to display question and its answers
				Set<Entry<SurveyQuestion, List<AnswerDTO>>> questionEntries = map
						.entrySet();
				int questionIdx = 0;
				for (Entry<SurveyQuestion, List<AnswerDTO>> questionEntry : questionEntries) {
					...
					cell = row.createCell(0);
					cell.setCellValue(resource
							.getMessage(MonitoringAction.MSG_LABEL_QUESTION)
							+ " " + questionIdx);
					cell = row.createCell(1);
					...
					cell = row.createCell(0);
					cell.setCellValue(resource
							.getMessage(MonitoringAction.MSG_LABEL_POSSIBLE_ANSWERS));
					...
					for (SurveyOption option : options) {
						...
						row = sheet.createRow(idx++);
						cell = row.createCell(0);
						cell.setCellValue(SurveyConstants.OPTION_SHORT_HEADER
								+ optionIdx);
						cell = row.createCell(1);
						cell.setCellValue(removeHTMLTags(option
								.getDescription()));
					}
					if (question.isAppendText()
						...
						cell = row.createCell(1);
						cell.setCellValue(resource
								.getMessage(MonitoringAction.MSG_LABEL_OPEN_RESPONSE));
					}
					...
					cell.setCellValue(resource
							.getMessage(MonitoringAction.MSG_LABEL_LOGIN));
					cellIdx++;
					cell = row.createCell(cellIdx);
					cell.setCellValue(resource
							.getMessage(MonitoringAction.MSG_LABEL_LEARNER_NAME));
					...
					for (AnswerDTO answer : answers) {
						...
						for (SurveyOption option : options) {
							...
							for (String choice : choices) {
								if (StringUtils.equals(choice, option.getUid()
										.toString())) {
									cell.setCellValue("X");
								}
							}
						...
						if (question.isAppendText()
							...
							cell = row.createCell(++cellIdx);
							if (answer.getAnswer() != null) {
								cell.setCellValue(removeHTMLTags(answer
			...
			errors = new ActionMessage("error.monitoring.export.excel",
					e.toString()).toString();
		}
	 ...
	 * @return
	 */
	public ActionForward setSubmissionDeadline(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) {
		...
		Long dateParameter = WebUtil.readLongParam(request,
				SurveyConstants.ATTR_SUBMISSION_DEADLINE, true);
		...
		if (dateParameter != null) {
			...
			tzSubmissionDeadline = DateUtil.convertFromTimeZoneToDefault(
					teacherTimeZone, submissionDeadline);
		}

...
public class AssessmentQuestionResultDAOHibernate extends BaseDAOHibernate implements AssessmentQuestionResultDAO {
	...
	return getHibernateTemplate().find(FIND_BY_ASSESSMENT_QUESTION_AND_USER,
		new Object[] { assessmentUid, userId, questionUid });
    }
	...
	List list = getHibernateTemplate().find(GET_ANSWER_MARK,
		new Object[] { assessmentUid, userId, questionSequenceId });
	if (list == null || list.size() == 0) {
	    return null;
	} else {

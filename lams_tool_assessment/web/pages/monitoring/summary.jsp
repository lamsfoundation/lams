...
<table class="alternative-color">
	<tr>
		...
		<td>
			<c:choose>
				<c:when test="${sessionMap.assessment.lockWhenFinished == true}">
	...
	<tr>
		...
		<td>
			<c:choose>
				<c:when test="${sessionMap.assessment.runAuto == true}">
					<fmt:message key="label.on" />
				</c:when>
				<c:otherwise>
					<fmt:message key="label.off" />
	...
	<tr>
		...
		<td>
			${sessionMap.assessment.miniViewAssessmentNumber}
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			<c:choose>
				<c:when test="${sessionMap.assessment.allowAddUrls == true}">
					<fmt:message key="label.on" />
				</c:when>
				<c:otherwise>
					<fmt:message key="label.off" />
	...
	<tr>
		...
		<td>
			<c:choose>
				<c:when test="${sessionMap.assessment.allowAddFiles == true}">
					<fmt:message key="label.on" />
				</c:when>
				<c:otherwise>
					<fmt:message key="label.off" />
	...
	<tr>
		...
		<td>
			<c:choose>
				<c:when test="${sessionMap.assessment.notifyTeachersOnAssigmentSumbit == true}">
					<fmt:message key="label.on" />
				</c:when>
				<c:otherwise>
					<fmt:message key="label.off" />
	...
	<tr>
		...
		<td>
			<c:choose>
				<c:when test="${sessionMap.assessment.reflectOnActivity == true}">
					<fmt:message key="label.on" />
				</c:when>
				<c:otherwise>
					<fmt:message key="label.off" />
	...
	<c:choose>
		<c:when test="${sessionMap.assessment.reflectOnActivity == true}">
			<tr>
				...
				<td>
					${sessionMap.assessment.reflectInstructions}	
				</td>
			</tr>
		</c:when>
	</c:choose>
	...
	<c:forEach var="group" items="${summaryList}" varStatus="firstGroup">
		...
		<c:forEach var="item" items="${group}" varStatus="status">
			...
			<c:if test="${item.itemUid != -1}">
				...
				<%-- Reflection list  --%>
				<c:if test="${sessionMap.assessment.reflectOnActivity && status.last}">

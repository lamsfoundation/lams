...
create table tl_lamind10_mindmap (uid bigint not null auto_increment, create_date datetime, update_date datetime, create_by bigint, title varchar(255), instructions text, run_offline bit, lock_on_finished bit, multiuser_mode bit, online_instructions text, offline_instructions text, content_in_use bit, define_later bit, tool_content_id bigint, last_action_id bigint, reflect_on_activity bit, reflect_instructions text, primary key (uid))type=innodb;
...
create table tl_lamind10_node (node_id bigint not null auto_increment, unique_id bigint, parent_id bigint, node_text varchar(100), node_color varchar(6), user_id bigint, mindmap_id bigint, primary key (node_id))TYPE=InnoDB;

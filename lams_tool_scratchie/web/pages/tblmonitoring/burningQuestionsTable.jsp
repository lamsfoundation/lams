...
<script>
	$(document).ready(function(){
		...
		$('.options-show-link').click(function(e){			
			// hide button, show answers
			let button = $(this),
				itemUid = button.closest('.collapse').data('itemUid');
			button.remove();
...
<div class="pb-2 clearfix">
	<c:if test="${burningQuestionsAvailable}">
		<button type="button" id="toggle-burning-questions-button" class="btn btn-secondary float-end" data-expanded="false">
				...
				<c:if test="${not empty burningQuestionItemDto.burningQuestionDtos}">
							...
							<c:set var="discussionItemUid" value="${empty item.uid ? generalBurningQuestionDiscussionItemUid : item.uid}" />
				 			<div class="accordion-item" tabindex="0">
							    ...
							    <div id="collapse${item.uid}" data-item-uid="${item.uid}" class="accordion-collapse collapse">	
									<div class="accordion-body">
										...
										<c:if test="${not empty burningQuestionItemDto.scratchieItem.uid}">
											<div class="clearfix">
												<button type="button" id="options-show-${item.uid}" class="btn btn-light btn-sm options-show-link float-end">
											...
											<table class="table table-striped">
												<tbody>
													<c:forEach var="burningQuestionDto" items="${burningQuestionItemDto.burningQuestionDtos}">
														<tr>
															<td class="text-nowrap align-middle fw-bold" width="20%">
															...
															<c:if test="${isTbl}">
																<td class="discussion-sentiment-start-button-cell align-middle">
																	<c:if test="${discussionSentimentEnabled and not empty discussionItemUid}">
																		<button type="button" id="discussion-sentiment-start-button-${discussionItemUid}-${burningQuestionDto.burningQuestion.uid}"
																		     	class="btn btn-sm btn-light discussion-sentiment-start-button"
																		     	onClick="startDiscussionSentiment(${discussionItemUid}, ${burningQuestionDto.burningQuestion.uid}, true)">
														...
														<c:if test="${isTbl and discussionSentimentEnabled and not empty discussionItemUid}">
															<tr id="discussion-sentiment-chart-row-${discussionItemUid}-${burningQuestionDto.burningQuestion.uid}"
																...
																<td colspan="4">
																	<div id="discussion-sentiment-chart-panel-container-${discussionItemUid}-${burningQuestionDto.burningQuestion.uid}"
																			class="w-75 mx-auto">
																	</div>
																</td>
															</tr>

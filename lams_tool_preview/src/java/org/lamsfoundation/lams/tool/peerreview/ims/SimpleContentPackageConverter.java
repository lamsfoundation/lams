 ...
 */
/* $$Id$$ */
package org.lamsfoundation.lams.tool.peerreview.ims;
...
public class SimpleContentPackageConverter implements IContentPackageConverter {
	...
	/** Built an XML document which is a list of organisations/peerreview. 
	 */ 
	 ...
	 * which has a peerreview or 
	 * (b) parentOrgIdentifier==defaultOrgIdentifier and it is the first item encountered 
	 * which has a peerreview. 
	 *
	 * @param element
	 * @return New version of element combining organization/item/peerreview details
	 */
	private Element processItem(Element element, Namespace nm, String defaultOrgIdentifier, String parentOrgIdentifier) 
		...
		if ( isVisible(isVisibleString) )  {
			...
			String peerreviewURL = getPeerreviewURL(element);
			if ( peerreviewURL != null )
				setAttribute(itref, OrganizationXMLDef.RESOURCE, peerreviewURL); // optional 

			if ( peerreviewURL != null && this.defaultItem == null && 
				...
				this.defaultItem = peerreviewURL;
			} else {
	...
	/* Get the peerreview relating to this item. First time called it will
	 * get a list of allowed peerreview it will cache the value in 
	 * cachedPeerreviewList. After that, it will go to the cache. If 
	 * it doesn't find the peerreview, then it will regenerate the list
	 * of peerreview.
	 * 
	 * This is done for performance - it is assumed that most items will 
	 * have access to the same peerreview. This may not be true if sub-manifests
	 * are used, hence the regneration of the array if the peerreview
	 ...
	 * @return relative path of the peerreview, null if no IDENTIFIERREF found
	 * @throws IMSManifestException if IDENTIFIERREF is not null but the peerreview
	 * could not found.
	 */
	private String getPeerreviewURL(Element itemElement) throws IMSManifestException {

		if ( cachedPeerreviewList == null ) {
			cachedPeerreviewList = cpCore.getReferencedElementsAllowed(itemElement);
			if ( log.isDebugEnabled() )
				log.debug("Peerreview are "+cachedPeerreviewList);
		}
		...
		if ( identifierRef == null ) {
			...
			Element peerreview = getPeerreview2(identifierRef);
			
			if ( peerreview == null ) {
				
				// We failed to find a matching peerreview so try generating the list again. 
				// Note: this may case the list to be generated twice first time if the 
				//peerreview is missing. Too bad!
				cachedPeerreviewList = cpCore.getReferencedElementsAllowed(itemElement);
				if ( log.isDebugEnabled() )
					log.debug("Peerreview are "+cachedPeerreviewList);
	
				peerreview = getPeerreview2(identifierRef);
			}
	
			if ( peerreview != null ) {
				return cpCore.getRelativeURL(peerreview);
			} else {
				throw new IMSManifestException("Unable to find peerreview for item element "
	 ...
	 * @return
	 */
	private Element getPeerreview2(String identifierRef) {
		for ( int i=0; i< cachedPeerreviewList.length; i++ ) {
			Element peerreview = cachedPeerreviewList[i];
			if ( identifierRef != null && 
					identifierRef.equals(peerreview.getAttributeValue(CP_Core.IDENTIFIER)) ) {
				return peerreview;
			}
		}
		return null;

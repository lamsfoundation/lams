...
public class AuthoringAction extends LamsDispatchAction {
	...
	protected ActionForward unspecified(ActionMapping mapping, ActionForm form,
		...
		if (wookieService == null) {
			wookieService = WookieServiceProxy.getWookieService(this
					.getServlet().getServletContext());
		}

		String wookieUrl = wookieService.getWookieURL();
		if (wookieUrl == null) {
			throw new WookieException(wookieService
					.getMessage("error.wookie.not.configured"));
		}
		...
		try {
			...
			if (widgetCount > 0) {
				int pages = 1;
				float pagesFloat = new Float(widgetCount)
						/ WookieConstants.WIDGETS_PER_PAGE;
				if (pagesFloat > 1) {
			...
			throw new WookieException(wookieService
					.getMessage("error.initiating.widget"), e);
		}
		...
		// Set up sessionMap
		SessionMap<String, Object> map = createSessionMap(wookie,
	...
	public ActionForward getWidgets(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
		...
		Integer pageNumber = WebUtil.readIntParam(request,
				WookieConstants.PARAM_PAGE_NUMBER);
		...
		try {
			...
			List<WidgetDefinition> widgetDefinitions = WookieUtil
					.getWidgetDefinitions(wookieUrl);
			List<WidgetDefinition> cutDownWidgetDefinitions = null;

			int startIndex = WookieConstants.WIDGETS_PER_PAGE
					* (pageNumber - 1);
			...
			if (widgetDefinitions.size() > startIndex) {
				if (widgetDefinitions.size() > endIndex) {
					...
					cutDownWidgetDefinitions = widgetDefinitions.subList(
							startIndex, endIndex);
				} else {
					// Use the start index to the end of the list
					cutDownWidgetDefinitions = widgetDefinitions.subList(
							startIndex, widgetDefinitions.size());
				}

			}
			request.setAttribute(WookieConstants.ATTR_WIDGET_LIST,
					cutDownWidgetDefinitions);
	...
	public ActionForward initiateWidget(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws WookieException {
		try {
			// set up wookieService
			...
			String wookieIdentifier = WebUtil.readStrParam(request,
					WookieConstants.PARAM_KEY_WIDGET_ID);
			Long toolContentID = new Long(WebUtil.readLongParam(request,
					AttributeNames.PARAM_TOOL_CONTENT_ID));

			wookieUrl += WookieConstants.RELATIVE_URL_WIDGET_SERVICE;

			String returnXML = WookieUtil
					.getWidget(wookieUrl, wookieKey, wookieIdentifier,
							getUser(), toolContentID.toString(), true);

			this.writeResponse(response, "text/xml;charset=utf-8", "utf-8",
					returnXML);
	...
	public ActionForward updateContent(ActionMapping mapping, ActionForm form,
		...
		ToolAccessMode mode = (ToolAccessMode) map
				.get(AuthoringAction.KEY_MODE);

		// get wookie content.
		Wookie wookie = wookieService.getWookieByContentId((Long) map
	 ...
	 * @return
	 */
	private void updateWookie(Wookie wookie, AuthoringForm authForm,
			ToolAccessMode mode) {
	 ...
	 * @param mode
	 */
	private SessionMap<String, Object> createSessionMap(Wookie wookie,
	...
	private UserDTO getUser() {
		return (UserDTO) SessionManager.getSession().getAttribute(
				AttributeNames.USER);
	}
}

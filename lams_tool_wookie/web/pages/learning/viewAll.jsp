...
<lams:html>
	...
	<body class="stripes">
			...
			<html:form action="/learning" method="post" styleId="messageForm">
					...
					<c:choose>
						<c:when test="${!empty learnerDTOs}">
							...
							<c:forEach var="learner" items="${learnerDTOs}">
								...
								<tr>
									<td width="30%">
										<a href="javascript:openPopup('${wookieImageFolderURL}/${learner.imageFileName}', ${learner.imageHeight}, ${learner.imageWidth})">
									...
									<td>
										<c:choose>
											<c:when test="${!learner.imageHidden}">
												<img src="${wookieImageFolderURL}/${learner.imageFileName}" 
													height="${learner.imageThumbnailHeight}" 
													width="${learner.imageThumbnailWidth}"
													title='<fmt:message key="tooltip.openfullsize" />'
													onclick="openPopup('${wookieImageFolderURL}/${learner.imageFileName}', ${learner.imageHeight}, ${learner.imageWidth})"
												/>
											</c:when>
											<c:otherwise>
				...
				&nbsp;
				<c:if test="${!wookieDTO.lockOnFinish}">
				...
				<c:choose>
					<c:when test="${wookieDTO.reflectOnActivity}">

...
public class ActivityMapping implements Serializable {
     ...
     *            if true then try to display the parallel activity frames rather than the
     ...
     * (1) the user has opened just the one activity in in a popup window from the progress bar, in which case we want
     * to close the window
     * (2) the user has opened up a parallel activity in a popup window from the progress bar, and the completed
     * activity
     * is one of the contained activities. In this case we want to display the "wait" message or close depending on the
     * activity
     * in the other frame.
     * (3) the activity was force completed while the user was doing the activity. This case includes "standalone"
     * activities + activities in a parallel frameset
     ...
     * onto the string.
     * If this is for a JSP call then the lessonID is needed, if it is for Flash then it is not needed as Flash will add
     * the id.
     */

...
public class LearnerAction extends LamsDispatchAction {
    ...
    /**
     * <p>
     * The struts dispatch action to retrieve the progress data from the server and tailor it into the object struture
     * that expected by flash. A wddx packet with object data struture is sent back in the end of this call. It is used
     * to construct or restore the flash learner progress bar
     * </p>
     * 
     * <p>
     * Gets the most recent copy from the database - not the cached version. That way if the cached version has
     * problems, at least we start off right!
     * </p>
     * 
     * <p>
     * As this process is expensive, the server is only expecting this call whenever is necessary. For example,
     * starting, resuming and restoring a new lesson. And it should not happen every time that learner is progressing to
     * next activity.
     * </p>
     * 
     * @param mapping
     ...
     * @throws ServletException
     */
    public ActionForward getFlashProgressData(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	    HttpServletResponse response) throws IOException, ServletException {
	if (LearnerAction.log.isDebugEnabled()) {

...
<style>
	.discussion-sentiment-chart-card .card-title .fa-comments {
		/* Override global LAMS CSS */
		color: black;
	}
	
	.discussion-sentiment-chart-card.disabled td,
	.discussion-sentiment-chart-card.disabled .card-title,
	.discussion-sentiment-chart-card.disabled .card-title .fa-comments {
		color: var(--bs-secondary);
	}

	.discussion-sentiment-chart-card .card-header {
		display: flex;
		justify-content: space-between;
	}
	
	.discussion-sentiment-chart-card .card-header .discussion-sentiment-start-button {
	...
	$(document).ready(function(){
		...
		var dataRefresh = window.setInterval(function(){
			var cardContent = $('#discussion-sentiment-chart-card-content-' + idSuffix),
				...
				existingDataRefresh = cardContent.data('dataRefresh');
			if (cardContent.length == 0 || cardContent.data('stop') || existingDataRefresh != dataRefresh) {

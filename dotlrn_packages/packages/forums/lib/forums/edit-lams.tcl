...
element create forum returnlams_url \
...
element create forum toolContentID \
    -label toolContentID -datatype integer -widget hidden 

forums::form::forum forum

if {[form is_valid forum]} {
    template::form get_values forum returnlams_url forum_id name charter presentation_type posting_policy new_threads_p toolContentID
    ...
    ad_returnredirect [export_vars -base $returnlams_url {dispatch toolContentID extToolContentID}]
    ad_script_abort
}

if { [form is_request forum] } {
    element set_properties forum returnlams_url -value $returnlams_url
    ...
    element set_properties forum toolContentID -value $toolContentID
}

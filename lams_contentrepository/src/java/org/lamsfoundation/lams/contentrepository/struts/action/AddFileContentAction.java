...
public class AddFileContentAction extends RepositoryDispatchAction {
    ...
    public ActionForward uploadFile(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	    ...
	    throws FileException, AccessDeniedException, InvalidParameterException, ItemNotFoundException {
	...
	if (log.isDebugEnabled()) {
	    log.debug("Adding file content " + filename + " type " + contentType + " version description "
		    + versionDescription + " for uuid " + uuid);
	}
	...
	try {
		...
		nodeKey = getRepository().updateFileItem(ticket, uuid, filename, is, contentType, versionDescription);
	    } else {
		// create a new node
		nodeKey = getRepository().addFileItem(ticket, is, filename, contentType, versionDescription);
	    }
     ...
     * @throws ItemNotFoundException
     */
    public ActionForward uploadPackage(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	    HttpServletResponse response)
	    throws FileException, AccessDeniedException, InvalidParameterException, ItemNotFoundException {
	...
	if (log.isDebugEnabled()) {
	    log.debug("Adding package content " + dirName + " start point " + entryString + " version description "
		    + versionDescription + " for uuid " + uuid);
	}
	...
	try {
	    ...
	    if (uuid != null) {
		// create a new version of this node
		nodeKey = getRepository().updatePackageItem(ticket, uuid, dirName, entryString, versionDescription);
	    } else {
		// create a new node

 ...
 * <p>
 * Runtime exceptions will trigger the transaction to be marked for rollback. Checked exceptions will NOT trigger the
 * transaction to be marked for rollback so it is up to the calling code to trigger a rollback if needed. This allows
 * the calling code to handle the exception and continue the transaction.
 */
public interface IRepositoryService {
    ...
    /**
     * Login, creating a new ticket for the given credentials and specified workspace. If login fails, a LoginException
     * is thrown and no valid ticket is generated. The credentials object in the ticket does not contain the password.
     *
    ...
    /**
     * Create a new workspace, with the tool identified in the creditials as the owner. It does not clear the password
     * in the credentials
     *
     * @param credentials
     ...
     *
     * At this stage it is publically accessable - may need to move it to a private management tool if considered to
     * insecure.
     *
     * @param newCredential
    ...
    /**
     * Add a new file to the repository. This will create a completely new entry (node) in the repository, starting with
     * version 1.
     *
     * @param ticket
     ...
     *
     * The directory separator character in the paths of the files in the package will be converted to "/" so that a web
     * style path can be used to access the file.
     *
     * @param ticket
     * 	ticket issued on login. Identifies tool and workspace - mandatory
     * @param dirPath
     ...
     * @param type
     * 	The type of the property. See org.lamsfoundation.lams.contentrepository.PropertyType for values.
     * @throws ValueFormatException
     * 	if the type or format of a value is incompatible with the type of the specified property or if value is
     * 	incompatible with (i.e. can not be converted to) type.
     */
    ...
    /**
     * Copy node, copying the specified/latest version of the existing node to version 1 of the new node.
     * <p>
     * Copies a node (identified by uuid/version). The file(s) attached to the supplied version are duplicated and
     * become version 1 of the new node. Therefore any "history" of the original node is lost. If the version is not
     * supplied, then it uses the latest version of the node as the basis for the new node.
     * <p>
    ...
    /**
     * Get an item from the repository based on the UUID and relative path. Only used to get the content from a package.
     * The UUID is the id of the package, and relPath is the relative path within the package.
     *
     ...
     * @param versionId
     * 	desired version - if null gets latest version. This is the version of the package node, not the related file.
     * @param relPath
     * 	relative path within the package - if null, returns start file.
     * @return node.
     * @throws AccessDeniedException
     * 	if ticket doesn't allow this action
     * @throws ItemNotFoundException
     ...
     * 	if any internal errors have occured
     */
    public abstract IVersionedNode getFileItem(ITicket ticket, String nodeId, Long versionId, String relPath)
	    throws AccessDeniedException, ItemNotFoundException, FileException;

    /**
     * Return a list of all the nodes for a package. The first in the list is the package node. The others are in
     * arbitrary order.
     *
     * @param ticket
     * @param uuid
    ...
    /**
     * Get the history for a node. Quite intensive operation the first time it is run on a node as it has to build all
     * the data structures.
     *
     * @param ticket
     * 	ticket issued on login. Identifies tool and workspace - mandatory
    ...
    /**
     * Delete the current version of a node, returning a list of the files that could not be deleted properly. If it is
     * a package node, the child nodes will be deleted.
     *
     * You cannot delete the child of a package node - you have to delete the whole package.
     *
     * If after deleting the version, we find that there a no other versions for a node, then delete the node.
     *
     * A file missing from the disk is ignored, allowing nodes with lost files to be deleted.
     *
     * @return the list of file(paths) that could not be deleted. The db entries will have been deleted but these files
    ...
    /**
     * Delete a node and all its versions, returning a list of the files that could not be deleted properly. If it is a
     * package node, the child nodes will be deleted.
     *
     * A file missing from the disk is ignored, allowing nodes with lost files to be deleted.
     *
     * @return the list of file(paths) that could not be deleted. The db entries will have been deleted but these files
    ...
    /**
     * Finish using this ticket. No more updates may be used with this ticket after logout(). Allows any resources to be
     * freed.
     *
     * @param ticket
     * 	ticket issued on login. Identifies tool and workspace - mandatory
    ...
    /**
     * Save current version of a node to local file by given file name. If the <code>toFileName</code> is null, file
     * name use original file name instead and file save path will be system temporary directory.
     * <p>
     *
     * @param ticket
     * 	ticket issued on login. Identifies tool and workspace - mandatory

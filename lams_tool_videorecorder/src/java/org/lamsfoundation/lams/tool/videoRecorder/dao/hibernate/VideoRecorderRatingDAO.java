...
public class VideoRecorderRatingDAO extends BaseDAO implements IVideoRecorderRatingDAO {
    ...
    private static final String SQL_QUERY_FIND_BY_RATING_ID = "from " + VideoRecorderRating.class.getName() + " as r"
	    + " where uid=?";
    ...
    private static final String SQL_QUERY_NB_RATINGS_BY_USER = "select count(*) from "
	    + VideoRecorderRecording.class.getName() + " r "
	    + " where r.createBy.userId=? and r.videoRecorderSession.sessionId=?";

    @Override
    public VideoRecorderRating getRatingById(Long ratingId) {
	List list = this.getHibernateTemplate().find(SQL_QUERY_FIND_BY_RATING_ID, ratingId);

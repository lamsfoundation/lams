...
public class ExportServlet extends AbstractExportPortfolioServlet {
    ...
    public static final String VIDEORECORDER_INCLUDES_HTTP_FOLDER_URL = Configuration.get(ConfigurationKeys.SERVER_URL)
	    + "tool/lavidr10/includes/flash/";
    public static final String VIDEORECORDER_RECORDINGS_HTTP_FOLDER_URL = Configuration
	    .get(ConfigurationKeys.SERVER_URL) + "tool/lavidr10/recordings/";
    public static final String VIDEORECORDER_RECORDINGS_FOLDER_DEST = Configuration.get(ConfigurationKeys.LAMS_EAR_DIR)
	    + "/" + "lams-tool-lavidr10.war" + "/" + "recordings" + "/";
    public static final String VIDEORECORDER_RECORDINGS_FOLDER_SRC = Configuration
	    .get(ConfigurationKeys.RED5_RECORDINGS_URL);
    ...
    private void doLearnerExport(HttpServletRequest request, HttpServletResponse response, String directoryName,
	    Cookie[] cookies) throws VideoRecorderException {
	...
	VideoRecorderSession videoRecorderSession = videoRecorderService.getSessionBySessionId(toolSessionID);
	...
	if (exportAll) {
	    ...
	    videoRecorderRecordingDTOs = videoRecorderService.getRecordingsByToolSessionIdAndUserUid(toolSessionID,
		    videoRecorderUser.getUid(), toolContentID);
	}
	...
	List<VideoRecorderRecordingDTO> videoRecorderRecordingDTOs = videoRecorderService
		.getRecordingsByToolSessionId(toolSessionID, toolContentID);
    ...
    private void bundleFilesForOnline(HttpServletRequest request, HttpServletResponse response, String directoryName,
	    Cookie[] cookies, List<VideoRecorderRecordingDTO> videoRecorderRecordingDTOs) {
    ...
    private void bundleFilesForOffline(HttpServletRequest request, HttpServletResponse response, String directoryName,
	    Cookie[] cookies, List<VideoRecorderRecordingDTO> videoRecorderRecordingDTOs) {
	// prepare 
	...
	try {
		...
		VideoRecorderRecordingDTO vr = iter.next();

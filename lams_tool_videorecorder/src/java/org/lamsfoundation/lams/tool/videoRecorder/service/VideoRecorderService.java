...
public class VideoRecorderService
	...
	VideoRecorder videoRecorder = videoRecorderDAO.getByContentId(toolContentId);
	for (VideoRecorderSession session : (Set<VideoRecorderSession>) videoRecorder.getVideoRecorderSessions()) {
	    if (!videoRecorderCommentDAO.getCommentsByToolSessionId(session.getSessionId()).isEmpty()
		    || !videoRecorderRatingDAO.getRatingsByToolSessionId(session.getSessionId()).isEmpty()
		    || !videoRecorderRecordingDAO.getByToolSessionId(session.getSessionId()).isEmpty()) {
		return true;
	    }
	}

...
public class LearningAction extends LamsDispatchAction {
	...
	if (videoRecorderSession == null) {
	    throw new VideoRecorderException("Cannot retrieve session with toolSessionID" + toolSessionID);
	}
	...
	// set readOnly flag.
	if (mode.equals(ToolAccessMode.TEACHER)
		|| (videoRecorder.isLockOnFinished() && videoRecorderUser.isFinishedActivity())) {
	...
	if (videoRecorderUser == null) {
	    VideoRecorderSession videoRecorderSession = videoRecorderService.getSessionBySessionId(toolSessionId);
	    videoRecorderUser = videoRecorderService.createVideoRecorderUser(user, videoRecorderSession);
	}
	...
	if (videoRecorderUser != null) {
		...
		videoRecorderUser.setEntryUID(videoRecorderService.createNotebookEntry(toolSessionID,
			CoreNotebookConstants.NOTEBOOK_TOOL, VideoRecorderConstants.TOOL_SIGNATURE,
			videoRecorderUser.getUserId().intValue(), learningForm.getEntryText()));
	    } else {
		// update existing entry.
		videoRecorderService.updateEntry(videoRecorderUser.getEntryUID(), learningForm.getEntryText());
	    }

	    videoRecorderUser.setFinishedActivity(true);
	    videoRecorderService.saveOrUpdateVideoRecorderUser(videoRecorderUser);
	} else {
	    log.error("finishActivity(): couldn't find VideoRecorderUser with id: " + videoRecorderUser.getUserId()
	...
	try {
	    nextActivityUrl = sessionMgrService.leaveToolSession(toolSessionID, videoRecorderUser.getUserId());

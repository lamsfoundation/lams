...
create table tl_lavidr10_attachment (uid bigint not null auto_increment, file_version_id bigint, file_type varchar(255), file_name varchar(255), file_uuid bigint, create_date datetime, videoRecorder_uid bigint, primary key (uid))type=innodb;
create table tl_lavidr10_videoRecorder (uid bigint not null auto_increment, create_date datetime, update_date datetime, create_by bigint, title varchar(255), instructions text, run_offline bit, lock_on_finished bit, online_instructions text, offline_instructions text, content_in_use bit, define_later bit, tool_content_id bigint, allow_use_voice bit, allow_use_camera bit, allow_learner_video_visibility bit, allow_learner_video_export bit, primary key (uid))type=innodb;
create table tl_lavidr10_session (uid bigint not null auto_increment, session_end_date datetime, session_start_date datetime, status integer, session_id bigint, session_name varchar(250), videoRecorder_uid bigint, primary key (uid))type=innodb;
create table tl_lavidr10_recording (uid bigint not null auto_increment, create_date datetime, update_date datetime, create_by bigint, title varchar(255), description varchar(1027), notes varchar(1027), rating float, filename varchar(255), videoRecorder_session_uid bigint, primary key (uid))type=innodb;
create table tl_lavidr10_user (uid bigint not null auto_increment, user_id bigint, last_name varchar(255), login_name varchar(255), first_name varchar(255), finishedActivity bit, videoRecorder_session_uid bigint, entry_uid bigint, primary key (uid))type=innodb;

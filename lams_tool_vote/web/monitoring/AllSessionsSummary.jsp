...
<c:forEach var="sessionDto" items="${voteGeneralMonitoringDTO.sessionDTOs}">
	...
	<c:if test="${isGroupedActivity}">
		<h2>
			<fmt:message key="label.groupName"/> <c:out value="${sessionDto.sessionName}"/>
		</h2>
		<br>
	</c:if>
	...
	<c:if test="${sessionDto.existsOpenVote == 'true' }">
		<table cellspacing="0" class="alternative-color">
			<tr>
			...
			<c:forEach var="dtoEntry" items="${sessionDto.listUserEntries}">

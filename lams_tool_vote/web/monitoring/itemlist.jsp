...
<table id="itemTable" style="align:left;width:650px" >
	...
	<tr>
			...
			<td width="10%" align="right">
		 		<c:if test="${totalNominationCount != 1}"> 		
			 		...
			 		<c:if test="${queIndex == 1}"> 		
						<a title="<bean:message key='label.tip.moveNominationDown'/>" href="javascript:;" onclick="javascript:submitModifyMonitoringNomination('<c:out value="${queIndex}"/>','moveNominationDown');">
		                            <img src="<c:out value="${tool}"/>images/down.gif" border="0">
						</a> 
					</c:if> 							
	
			 		<c:if test="${queIndex == totalNominationCount}"> 		
						<a title="<bean:message key='label.tip.moveNominationUp'/>" href="javascript:;" onclick="javascript:submitModifyMonitoringNomination('<c:out value="${queIndex}"/>','moveNominationUp');">
		...
		<td width="10%" align="right">
				<a title="<bean:message key='label.tip.deleteNomination'/>" href="javascript:;" onclick="removeMonitoringNomination(${queIndex});">

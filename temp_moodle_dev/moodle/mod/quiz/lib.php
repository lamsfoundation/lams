 ...
 */
function quiz_clone_instance($id, $sectionref, $courseid) {
	global $CFG;
	...
	if ( ! $cm ) {
        ...
        $existingquiz->old_id = $existingquiz->id;
...
/**
 * LAMS Function
 * Serialize a quiz instance and return serialized string to LAMS
 */
function quiz_export_instance($id) {
    $cm = get_record('course_modules', 'id', $id);
    if ($cm) {
        if ($quiz = get_record('quiz', 'id', $cm->instance)) {
            // serialize quiz into a string; assuming quiz object
...
/**
 * LAMS Function
 * Deserializes a serialized Moodle quiz, and creates a new instance of it
 */
function quiz_import_instance($filepath, $userid, $courseid, $sectionid) {
    // file contents contains serialized quiz object
    $filestr = file_get_contents($filepath);
    $quiz = unserialize($filestr);

    // import this quiz into a new course
    $quiz->course = $courseid;

    // escape text columns for saving into database
    $quiz->name = addslashes($quiz->name);
    $quiz->intro = addslashes($quiz->intro);
...
/**
 * LAMS Function
 * Return a statistic for a given user in this Moodle quiz for use in branching
 */
...
/**
 * LAMS Function
 * Return an HTML representation of a user's activity in this quiz
 */
function quiz_export_portfolio($id, $userid) {
    global $CFG;
    $text = 'This tool does not support export portfolio.  It may be accessible via ';
    $text .= '<a href="'.$CFG->wwwroot.'/mod/quiz/view.php?id='.$id.'">this</a> link.';
    return $text;

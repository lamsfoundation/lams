<?php
 	...
 	/**
 	 * This page is a Moodle wiki tool adapter interface for LAMS to call.  It
 	 * is secured by the LAMS - Moodle authentication mechanism.
 	 */
 	    include_once('../../config.php');
 	    include_once($CFG->libdir.'/datalib.php');
 	    include_once('../wiki/lib.php');
 	    ...
 	    switch ($method) {
 	        case 'clone':
 	            $newcmid = wiki_clone_instance($cmid, $sectionid, $courseid,$user->id);
 	            echo $newcmid;
 	            break;
 	        case 'import':
 	            $newcmid = wiki_import_instance($_FILES['upload_file']['tmp_name'], $user->id, $courseid, $sectionid);
 	            echo $newcmid;
 	            break;
 	        case 'export':
 	            wiki_export_instance($cmid);
 	            break;
 	        case 'export_portfolio':
 	            $text = wiki_export_portfolio($cmid, $user->id);
 	            echo $text;
 	            break;
 	        case 'output':
 	            $output = wiki_get_tool_output($cmid,$outputname,$user->id);
 	            echo $output;
 	            break;
 	        default:
 	    }

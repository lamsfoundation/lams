<?php
 	...
 	/**
 	 * This page is a Moodle chat tool adapter interface for LAMS to call.  It
 	 * is secured by the LAMS - Moodle authentication mechanism.
 	 */
 	    include_once('../../config.php');
 	    include_once($CFG->libdir.'/datalib.php');
 	    include_once('../chat/lib.php');
 	    ...
 	    switch ($method) {
 	        case 'clone':
 	            $newcmid = chat_clone_instance($cmid, $sectionid, $courseid);
 	            echo $newcmid;
 	            break;
 	        case 'import':
 	            $newcmid = chat_import_instance($_FILES['upload_file']['tmp_name'], $user->id, $courseid, $sectionid);
 	            echo $newcmid;
 	            break;
 	        case 'export':
 	            chat_export_instance($cmid);
 	            break;
 	        case 'export_portfolio':
 	            $text = chat_export_portfolio($cmid, $user->id);
 	            echo $text;
 	            break;
 	        case 'output':
 	            $output = chat_get_tool_output($cmid, $user->id);
 	            echo $output;
 	            break;
 	        default:
 	    }

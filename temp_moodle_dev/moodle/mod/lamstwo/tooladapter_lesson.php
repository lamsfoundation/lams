<?php
 	...
 	/**
 	 * This page is a Moodle lesson tool adapter interface for LAMS to call.  It
 	 * is secured by the LAMS - Moodle authentication mechanism.
 	 */
 	    include_once('../../config.php');
 	    include_once($CFG->libdir.'/datalib.php');
 	    include_once('../lesson/lib.php');
 	    ...
 	    switch ($method) {
 	        case 'clone':
 	            $newcmid = lesson_clone_instance($cmid, $sectionid, $courseid,$user->id);
 	            echo $newcmid;
 	            break;
 	        case 'import':
 	            $newcmid = lesson_import_instance($_FILES['upload_file']['tmp_name'], $user->id, $courseid, $sectionid);
 	            echo $newcmid;
 	            break;
 	        case 'export':
 	            lesson_export_instance($cmid);
 	            break;
 	        case 'export_portfolio':
 	            $text = lesson_export_portfolio($cmid, $user->id);
 	            echo $text;
 	            break;
 	        case 'output':
 	            $output = lesson_get_tool_output($cmid, $user->id);
 	            echo $output;
 	            break;
 	        default:
 	    }

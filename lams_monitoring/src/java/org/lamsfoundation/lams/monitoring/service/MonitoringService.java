...
public class MonitoringService implements IMonitoringService, ApplicationContextAware {
	...
	public Lesson initializeLesson(String lessonName, String lessonDescription, Boolean learnerExportAvailable,
		...
		LearningDesign copiedLearningDesign = authoringService.copyLearningDesign(originalLearningDesign, new Integer(copyType),
				user, workspaceFolder, true, null, customCSV);
	...
	public String createLessonClassForLessonWDDX(Integer creatorUserId, String lessonPacket) throws UserAccessDeniedException {
		...
		try {
			...
			//Create Lesson class
			createLessonClassForLesson(lessonId, organisation, learnerGroupName, learnerList, staffGroupName, staffList,
					creatorUserId);
	...
	public void startLessonOnSchedule(long lessonId, Date startDate, Integer userId) {
		...
		if (requestedLesson.isLessonStarted()) {
			...
			MonitoringService.log.error("Lesson for id=" + lessonId + " has been started. Unable to schedule lesson start.");
			return;
		}

		if (requestedLesson.getScheduleStartDate() != null) {
			// can't reschedule!
			MonitoringService.log.error("Lesson for id=" + lessonId + " is already scheduled and cannot be rescheduled.");
			return;
		}
		...
		Trigger startLessonTrigger = new SimpleTrigger("startLessonOnScheduleTrigger:" + lessonId, Scheduler.DEFAULT_GROUP,
				startDate);
	...
	public void finishLessonOnSchedule(long lessonId, Date endDate, Integer userId) {
		...
		//create customized triggers
		Trigger finishLessonTrigger = new SimpleTrigger("finishLessonOnScheduleTrigger:" + lessonId, Scheduler.DEFAULT_GROUP,
				endDate);
		//start the scheduling job
		try {
	...
	public Integer startSystemActivity(Activity activity, Integer currentMaxId, Date lessonStartTime, String lessonName) {
		...
		if (activity.isBranchingActivity() && activity.getGrouping() == null) {
			...
			activity.setGrouping(grouping);
			if (MonitoringService.log.isDebugEnabled()) {
				MonitoringService.log.debug("startLesson: Created chosen grouping " + grouping + " for branching activity "
						+ activity);
			}
	...
	public ScheduleGateActivity runGateScheduler(ScheduleGateActivity scheduleGate, Date schedulingStartTime, String lessonName) {
		...
		//create customized triggers
		Trigger openGateTrigger = new SimpleTrigger("openGateTrigger:" + scheduleGate.getActivityId(), Scheduler.DEFAULT_GROUP,
				scheduleGate.getLessonGateOpenTime(schedulingStartTime));

		Trigger closeGateTrigger = new SimpleTrigger("closeGateTrigger:" + scheduleGate.getActivityId(), Scheduler.DEFAULT_GROUP,
		...
		if (gate != null) {
			...
			if (gate.isScheduleGate()) {
				...
				try {
					...
					MonitoringService.log.error("Error unscheduling trigger for gate activity id:" + gate.getActivityId(), e);
	...
	*/
	private String forceCompleteActivity(User learner, Long lessonId, LearnerProgress progress, Activity activity,
			...
			if (myGroup == null || myGroup.isNull()) {
				...
				if (grouping.isRandomGrouping()) {
					...
					try {
						...
						MonitoringService.log.error("Force complete failed. Learner " + learner + " lessonId " + lessonId
								+ " processing activity " + activity, e);
						stopReason = messageService.getMessage(MonitoringService.FORCE_COMPLETE_STOP_MESSAGE_GROUPING_ERROR,
		...
		if (stopReason == null) {
			...
			if (learnerProgress.getCompletedActivities().contains(stopActivity)) {
				...
				stopReason = messageService.getMessage(MonitoringService.FORCE_COMPLETE_STOP_MESSAGE_COMPLETED_TO_ACTIVITY,
						new Object[] { activity.getTitle() });
	...
	public String getLessonDetails(Long lessonID, Integer userID) throws IOException {
		...
		if (dto.getStartDateTime() != null) {
			dto.setStartDateTimeStr(DateFormat.getDateTimeInstance(DateFormat.FULL, DateFormat.FULL, userLocale).format(
					dto.getStartDateTime()));
		}

		if (dto.getScheduleStartDate() != null) {
			dto.setScheduleStartDateStr(DateFormat.getDateTimeInstance(DateFormat.FULL, DateFormat.FULL, userLocale).format(
					dto.getScheduleStartDate()));
		}
	...
	public String getAdditionalLearnersProgress(Long lessonID, Integer lastUserID, Integer userID) throws IOException {
		...
		if (lesson != null) {
			...
			flashMessage = new FlashMessage("getAdditionalLearnersProgress", messageService.getMessage("NO.SUCH.LESSON",
					new Object[] { lessonID }), FlashMessage.ERROR);
		}
		return flashMessage.serializeMessage();
	}
	...
	public String getAllContributeActivities(Long lessonID) throws IOException, LearningDesignProcessorException {
		...
		FlashMessage flashMessage;
		if (lesson != null) {
			ContributeActivitiesProcessor processor = new ContributeActivitiesProcessor(lesson.getLearningDesign(), lessonID,
					activityDAO, lamsCoreToolService);
	...
	public String getLearnerActivityURL(Long lessonID, Long activityID, Integer learnerUserID, Integer requestingUserId)
		...
		if (activity == null || learner == null) {
			...
			url = "monitoring/complexProgress.do?" + AttributeNames.PARAM_ACTIVITY_ID + "=" + activityID + "&"
					+ AttributeNames.PARAM_LESSON_ID + "=" + lessonID + "&" + AttributeNames.PARAM_USER_ID + "=" + learnerUserID;
		}
	...
	public String getActivityDefineLaterURL(Long lessonID, Long activityID, Integer userID) throws IOException,
		...
		Activity activity = activityDAO.getActivityByActivityId(activityID);
		if (activity == null) {
			MonitoringService.log.error("getActivityMonitorURL activity missing. Activity ID " + activityID + " activity "
					+ activity);
	...
	public String moveLesson(Long lessonID, Integer targetWorkspaceFolderID, Integer userID) throws IOException {
		...
		if (lesson != null) {
			...
			flashMessage = new FlashMessage("moveLesson", messageService.getMessage("NO.SUCH.LESSON", new Object[] { lessonID }),
					FlashMessage.ERROR);
	...
	*/
	public String renameLesson(Long lessonID, String newName, Integer userID) throws IOException {
		...
		if (lesson != null) {
			...
			flashMessage = new FlashMessage("renameLesson", messageService
					.getMessage("NO.SUCH.LESSON", new Object[] { lessonID }), FlashMessage.ERROR);
		}
		return flashMessage.serializeMessage();
	}
	...
	*/
	 ...
	 * @param newLesson 
	 */
	private LessonClass createLessonClass(Organisation organisation, String learnerGroupName, List<User> organizationUsers,
			String staffGroupName, List<User> staffs, Lesson newLesson) {
	...
	public int deleteAllOldPreviewLessons() {
		...
		while (iter.hasNext()) {
			...
			try {
				...
				MonitoringService.log.error("Unable to delete lesson " + lesson.getLessonId() + " due to exception.", e);
			}
		}
	 ...
	 */
	public void addGroup(Long activityID, String name, boolean overrideMaxNumberOfGroups) throws LessonServiceException,
		...
		if (overrideMaxNumberOfGroups) {
			...
			if (grouping.getMaxNumberOfGroups() != null && grouping.getMaxNumberOfGroups() > 0 && grouping.getGroups() != null
				...
				if (!usedForBranching) {
					...
					throw new MonitoringServiceException("Cannot increase the number of groups for the grouping " + grouping
	...
	public void addUsersToBranch(Long sequenceActivityID, String learnerIDs[]) throws LessonServiceException {
		...
		if (group == null) {
			...
			if (parentActivity == null || !parentActivity.isBranchingActivity()) {
				String error = "addUsersToBranch: Branching activity missing or not a branching activity. Branch was " + branch
	...
	public boolean isActivityAttempted(Activity activity) {
		...
		if (MonitoringService.log.isDebugEnabled()) {
			MonitoringService.log.debug("isActivityAttempted: num attempts for activity " + activity.getActivityId() + " is "
					+ numAttempted);
		}

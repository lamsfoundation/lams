...
public class MonitoringAction extends LamsDispatchAction {
    ...
    //---------------------------------------------------------------------
    /**
     * This STRUTS action method will initialize a lesson for specific learning
     * design with the given lesson title and lesson description.
     * <p>
     * If initialization is successed, this method will return a WDDX message
     * which includes the ID of new lesson.
     * 
     * @param mapping
	...
	try {
	    ...
	    Lesson newLesson = monitoringService.initializeLesson(title, desc, learnerExportAvailable, ldId,
		    organisationId, getUserId(), null, learnerPresenceAvailable, learnerImAvailable, liveEditEnabled);
    ...
    /**
     * The Struts dispatch method that starts a lesson on schedule that has been
     * created beforehand.
     * 
     * @param mapping
     *                An ActionMapping class that will be used by the Action
    ...
    /**
     * The Struts dispatch method that finsh a lesson on schedule that has been
     * started beforehand.
     * 
     * @param mapping
     *                An ActionMapping class that will be used by the Action
    ...
    /**
     * The Struts dispatch method to archive a lesson. A wddx acknowledgement
     ...
     * its previous state. A wddx acknowledgement message will be serialized and
     * sent back to the flash component.
     * 
     * @param mapping
     *                An ActionMapping class that will be used by the Action
     *                class to tell the ActionServlet where to send the
    ...
    /**
     * The purpose of suspending is to hide the lesson from learners
     * temporarily. It doesn't make any sense to suspend a created or a not
     * started (ie scheduled) lesson as they will not be shown on the learner
     * interface anyway! If the teacher tries to suspend a lesson that is not in
     * the STARTED_STATE, then an error should be returned to Flash.
     * 
     * @param mapping
     * @param form
     * @param request
    ...
    /**
     * Unsuspend a lesson which state must be Lesson.SUPSENDED_STATE. Otherwise
     * a error message will return to flash client.
     * 
     * @param mapping
     * @param form
     * @param request
     ...
     * by the given lesson ID as <code>Lesson.REMOVED_STATE</code> status.
     * </P>
     * <P>
     ...
     * </P>
     * <P>
     * This action need a lession ID, Learner ID and Activity ID as input.
     * Activity ID is optional, if it is null, all activities for this learner
     * will complete to as end as possible.
     * </P>
     * 
     * @param form
     * @param request
    ...
    /**
     * Get the learner progress data for all learners in a lesson. This is
     * called by the sequence tab in monitoring
     */
    ...
    /**
     * Get the first batch of learner progress data learners in a lesson. This
     * is called by the learner progress tab in monitoring
     */
    ...
    /**
     * Get a followup batch of learner progress data learners in a lesson. This
     * is called by the learner progress tab in monitoring
     */
    ...
    /**
     * Calls the server to bring up the learner progress page. Assumes
     * destination is a new window. The userid that comes from Flash is the user
     * id of the learner for which we are calculating the url. This is different
    ...
    /**
     * Calls the server to bring up the activity's monitoring page. Assumes
     * destination is a new window
     */
    ...
    /**
     * Calls the server to bring up the activity's define later page. Assumes
     * destination is a new window
     */
	...
	try {
	    ...
	    /**
	     * InitializeLessonServlet handles the Lesson initialisation
	     * process. Create Lesson Class and start Lesson Preview.
	     */
     ...
     * organisations. Should go to a monitoring webservice maybe ?
     */
     ...
     * Expects parameters lessonID and learnerExportPortfolio.
     */
    ...
    /**
     * Set whether or not the presence available button is available in learner.
    ...
    /**
     * Set whether or not the presence available button is available in learner.
     * Expects parameters lessonID and presenceImAvailable.
     */

...
public class EadventurePedagogicalPlannerForm extends PedagogicalPlannerActivityForm {
	...
	if (title != null && !title.isEmpty()) {
		...
		if (!StringUtils.isEmpty(title.get(index))) {
		    ...
		    if (itemType.equals(EadventureConstants.RESOURCE_TYPE_URL) && StringUtils.isEmpty(url.get(index))) {
			ActionMessage error = new ActionMessage("error.planner.url.blank", index + 1);
			errors.add(ActionMessages.GLOBAL_MESSAGE, error);

		    } else if (itemType.equals(EadventureConstants.RESOURCE_TYPE_FILE)) {
			/*

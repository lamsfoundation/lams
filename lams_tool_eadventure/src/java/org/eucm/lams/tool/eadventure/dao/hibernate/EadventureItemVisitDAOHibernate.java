...
public class EadventureItemVisitDAOHibernate extends BaseDAOHibernate implements EadventureItemVisitDAO {
    ...
    private static final String FIND_VIEW_COUNT_BY_USER = "select count(*) from "
	    + EadventureItemVisitLog.class.getName() + " as r where  r.sessionId=? and  r.user.userId =?";

    private static final String FIND_SUMMARY = "select v.eadventure.uid, count(v.eadventure) from  "
	    + EadventureItemVisitLog.class.getName() + " as v , " + EadventureSession.class.getName() + " as s, "
	    + Eadventure.class.getName() + "  as r " + " where v.sessionId = s.sessionId "
	    + " and s.eadventure.uid = r.uid " + " and r.contentId =? " + " group by v.sessionId, v.eadventure.uid ";

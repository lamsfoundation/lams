...
public class NotebookUserDAO extends BaseDAO implements INotebookUserDAO {
	...
	public List<Object[]> getUsersForTablesorter(final Long sessionId, int page, int size, int sorting, String searchString, 
	    ...
	    String[] notebookEntryStrings = coreNotebookService.getNotebookEntrySQLStrings(sessionId.toString(),NotebookConstants.TOOL_SIGNATURE,"user.user_id",true );
	    ...
	    queryText.append(" JOIN tl_lantbk11_session session ON user.notebook_session_uid = session.uid and session.session_id = :sessionId");
		...
		.addScalar("notebookCreateDate", Hibernate.TIMESTAMP)
		.addScalar("notebookModifiedDate", Hibernate.TIMESTAMP)
	...
	public List<StatisticDTO> getStatisticsBySession(final Long contentId) {
	    ...
	    query.addScalar("sessionId", Hibernate.LONG)
		.addScalar("sessionName", Hibernate.STRING)
		.addScalar("numLearners", Hibernate.INTEGER)
		.addScalar("numLearnersFinished", Hibernate.INTEGER)

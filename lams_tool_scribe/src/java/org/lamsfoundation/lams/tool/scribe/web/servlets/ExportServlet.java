...
public class ExportServlet extends AbstractExportPortfolioServlet {
	...
	try {
		...
		writeResponseToFile(basePath + "/pages/export/learner.jsp", directoryName, FILENAME, cookies);
    ...
    private void doLearnerExport(HttpServletRequest request, HttpServletResponse response, String directoryName,
	    Cookie[] cookies) throws ScribeException {
	...
	for (Iterator iter = scribe.getScribeSessions().iterator(); iter.hasNext();) {
	    ...
	    int numberOfVotes = 0;
	    for (Iterator iterator = session.getScribeUsers().iterator(); iterator.hasNext();) {

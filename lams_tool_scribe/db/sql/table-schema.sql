alter table tl_lascrb11_attachment drop foreign key FK57953706B3FA1495;
...
create table tl_lascrb11_heading (uid bigint not null auto_increment, heading longtext, scribe_uid bigint, display_order integer, primary key (uid));
create table tl_lascrb11_report_entry (uid bigint not null auto_increment, entry_text longtext, scribe_heading_uid bigint, scribe_session_uid bigint, primary key (uid));
create table tl_lascrb11_scribe (uid bigint not null auto_increment, create_date datetime, update_date datetime, create_by bigint, title varchar(255), instructions longtext, run_offline bit, lock_on_finished bit, auto_select_scribe bit, reflect_on_activity bit, reflect_instructions longtext, online_instructions longtext, offline_instructions longtext, content_in_use bit, define_later bit, tool_content_id bigint, aggregated_reports bit, primary key (uid));
